...
;      5.3     call to PUSH_FAC1_SIGN may return to an address -$100 (page not incremented)
...
;      5.8j    Fix PUSH_FAC1_SIGN if we hit page edge
...
INIT_COPY_PG2_LOOP:
    ...
    dey                         ; decrement count
    bpl INIT_COPY_PG2_LOOP      ; loop if not done
...
INIT_COPY_ZP_LOOP:
    ...
    dex                         ; decrement count
    bne INIT_COPY_ZP_LOOP       ; loop if not all done
...
; these two lines are only needed if Ram_base is $xxFF
    bne INIT_NO_PAGE_ROLLOVER   ; branch if no rollover
    inc Smemh                   ; increment start of mem high byte
INIT_NO_PAGE_ROLLOVER:

    jsr LAB_CRLF                ; print CR/LF
    jsr DO_NEW_CLEAR            ; do "NEW" and "CLEAR"
    ...
    jsr PRINT_XA_UNSIGNED       ; print XA as unsigned integer (bytes free)
    lda #<LAB_SMSG              ; point to sign-on message (low addr)
    ldy #>LAB_SMSG              ; point to sign-on message (high addr)
    jsr PRINT_STR_FROM_AY       ; print null terminated string from memory
...
OPEN_MEM_SPACE:
    jsr CHK_AVAIL_MEM           ; check available memory, "Out of memory" error if no room
...
OPEN_MEM_NO_EARRY:
    ...
    beq BLOCKMOVE_DEC_PTRS      ; branch if length low byte=0
...
; block is (X-1)*256+Y bytes, do the Y bytes first
    ...
    bcs BLOCKMOVE_ADJDST        ; branch if no underflow
...
BLOCKMOVE_ADJDST:
    ...
    bcs BLOCKMOVE_DEC_IDX       ; branch if no underflow

    dec Nbendh                  ; else decrement block end high byte
    bcc BLOCKMOVE_DEC_IDX       ; branch always
...
BLOCKMOVE_DEC_IDX:
    dey                         ; decrement index
    bne BLOCKMOVE_COPY_BYTE     ; loop until Y=0
...
BLOCKMOVE_DEC_PTRS:
    ...
    bne BLOCKMOVE_DEC_IDX       ; loop until count = $0
...
CHK_AVAIL_MEM:
    ...
    bcc MEM_OK_RTS              ; if less then exit (is ok)

    bne SAVE_NUMERIC_WORK       ; skip next test if greater (tested <)
...
SAVE_WORK_LOOP:
    ...
    dex                         ; decrement index
    bpl SAVE_WORK_LOOP          ; loop until all done
...
RESTORE_WORK_LOOP:
    ...
    bmi RESTORE_WORK_LOOP       ; loop if more to do
...
LAB_XERR:
    ...
    jsr FLUSH_STACK_CLR_CONT    ; flush stack and clear continue flag
...
; else print line number
    jsr PRINT_IN_LINE           ; print " in line [LINE #]"
...
; clear ON IRQ/NMI bytes
    ...
    jsr PRINT_STR_FROM_AY       ; go do print string

; wait for Basic command (no "Ready")
LAB_MAIN:
    jsr BASIC_INPUT_ENTRY       ; call for BASIC input
SET_EXEC_PTR_XY:
...
; got to interpret input line now ..
    ...
    bcc HANDLE_NEW_LINE         ; branch if numeric character (handle new BASIC line)

; no line number .. immediate mode
    jsr CRUNCH_KEYWORDS         ; crunch keywords into Basic tokens
    jmp INTERP_INCR_AND_SCAN    ; go scan and interpret code
...
HANDLE_NEW_LINE:
    ...
    bcc NEW_LINE_NOT_EMPTY      ; branch if not found
...
; aroooogah! line # already exists! delete it
    ...
    bcs CLOSEUP_MEM_SETUP       ; branch if overflow
...
CLOSEUP_MEM_SETUP:
    ...
    bcc CLOSEUP_COPY_LOOP       ; branch if no overflow
...
CLOSEUP_COPY_LOOP:
    ...
    iny                         ; increment index
    bne CLOSEUP_COPY_LOOP       ; while <> 0 do this block

    inc ut1_ph                  ; increment source pointer high byte
    inc ut2_ph                  ; increment destination pointer high byte
    dex                         ; decrement block count
    bne CLOSEUP_COPY_LOOP       ; loop until all done

; got new line in buffer and no existing same #
NEW_LINE_NOT_EMPTY:
    lda Ibuffs                  ; get byte from start of input buffer
    beq REBUILD_LINE_CHAIN      ; if null line just go flush stack/vars and exit
...
; got new line and it isn't empty line
    ...
    bcc NEW_BLOCK_HI_OK         ; branch if no overflow from add
...
NEW_BLOCK_HI_OK:
    ...
    jsr OPEN_MEM_SPACE          ; open up space in memory
...
COPY_CRUNCHED_LINE:
    ...
    bne COPY_CRUNCHED_LINE      ; continue while count <> 3
...
REBUILD_LINE_CHAIN:
    jsr RESET_EXEC_CLEAR_VARS   ; reset execution to start, clear vars and flush stack
...
LINE_CHAIN_LOOP:
    ...
    lda (ut1_pl),Y              ; get it
    beq GOTO_MAIN_WAIT          ; exit if end of program
...
SCAN_EOL_LOOP:
    ...
    bne SCAN_EOL_LOOP           ; loop if not [EOL]
    ...
    bcc LINE_CHAIN_LOOP         ; go do next line, branch always, carry clear
...
LAB_INLN:
    jsr PRINT_QUESTION_CHAR     ; print "?" character
    jsr PRINT_SPACE_CHAR        ; print " "
    bne BASIC_INPUT_ENTRY       ; call for BASIC input and return
...
INPUT_POLL_LOOP:
    ...
    bcc INPUT_POLL_LOOP         ; loop if no byte

    beq INPUT_POLL_LOOP         ; loop until valid input (ignore NULLs)

    cmp #$07                    ; compare with [BELL]
    beq INPUT_BUF_NOT_FULL      ; branch if [BELL]

    cmp #$0D                    ; compare with [CR]
    beq INPUT_CR_EXIT           ; do CR/LF exit if [CR]

    cpx #$00                    ; compare pointer with $00
    bne INPUT_BUF_NOT_EMPTY     ; branch if not empty

; next two lines ignore any non print character and [SPACE] if input buffer empty
    cmp #$21                    ; compare with [SP]+1
    bcc INPUT_POLL_LOOP         ; if < ignore character

INPUT_BUF_NOT_EMPTY:
    cmp #$08                    ; compare with [BACKSPACE] (delete last character)
    beq DELETE_LAST_CHAR        ; go delete last character

INPUT_BUF_NOT_FULL:
    cpx #Ibuffe-Ibuffs          ; compare character count with max
    bcs INPUT_BUF_FULL_BELL     ; skip store and do [BELL] if buffer full

    sta Ibuffs,X                ; else store in buffer
    inx                         ; increment pointer
INPUT_PRINT_LOOP:
    jsr LAB_PRNA                ; go print the character
    bne INPUT_POLL_LOOP         ; always loop for next character

INPUT_CR_EXIT:
    jmp INPUT_CR_LF_RETURN      ; do CR/LF exit to BASIC

; announce buffer full
INPUT_BUF_FULL_BELL:
    lda #$07                    ; [BELL] character into A
    bne INPUT_PRINT_LOOP        ; go print the [BELL] but ignore input character
; branch always
...
; *** end
    ...
    bcs CRUNCH_SAVE_BYTE        ; if >= go save byte then continue crunching

    cmp #'<'                    ; compare with "<"
    bcs CRUNCH_NOW              ; if >= go crunch now
    ...
    beq CRUNCH_COPY_QUOTED      ; branch if so (copy quoted string)

    cmp #'*'                    ; compare with "*"
    bcc CRUNCH_SAVE_BYTE        ; if < go save byte then continue crunching

; else crunch now
CRUNCH_NOW:
    bit Oquote                  ; get open quote/DATA token flag
    bvs CRUNCH_SAVE_BYTE        ; branch if b6 of Oquote set (was DATA)
...
CRUNCH_FIRST_CHAR_LOOP:
    cmp (ut2_pl),Y              ; compare with keyword first character table byte
    beq CRUNCH_MATCHED_FIRST    ; go do word_table_chr if match

; *** replace
;   bcc   CRUNCH_RESTORE_IDX         ; if < keyword first character table byte go restore
...
; *** end
    ...
    bne CRUNCH_FIRST_CHAR_LOOP  ; and loop (branch always)

; have matched first character of some keyword
CRUNCH_MATCHED_FIRST:
...
CRUNCH_TEST_KWD_END:
    bmi CRUNCH_RESTORE_IDX      ; all bytes matched so go save token
...
; *** end
    beq CRUNCH_KWD_CMP_LOOP     ; go compare next if match

    bne CRUNCH_KWD_NO_MATCH     ; branch if >< (not found keyword)
...
CRUNCH_SAVE_BYTE:
    ...
    beq CRUNCH_REACHED_EOL      ; do exit if was null [EOL]

; A holds token or byte here
    sbc   #':'              ; subtract ":" (carry set by CMP #00)
    beq   CRUNCH_COLON_OR_DATA  ; branch if it was ":" (is now $00)

; A now holds token-$3A
    cmp #token_DATA-$3A         ; compare with DATA token - $3A
    bne CRUNCH_CHK_REM          ; branch if not DATA

; token was : or DATA
CRUNCH_COLON_OR_DATA:
    sta   Oquote            ; save token-$3A (clear for ":", token_DATA-$3A for DATA)
CRUNCH_CHK_REM:
    eor #token_REM-$3A          ; effectively subtract REM token offset
    bne CRUNCH_LOOP_TOP         ; If wasn't REM then go crunch rest of line

    sta Asrch                   ; else was REM so set search for [EOL]

; loop for REM, "..." etc.
CRUNCH_REM_COPY_LOOP:
    lda Ibuffs,X                ; get byte from input buffer
    beq CRUNCH_SAVE_BYTE        ; branch if null [EOL]

    cmp Asrch                   ; compare with stored character
    beq CRUNCH_SAVE_BYTE        ; branch if match (end quote)
...
CRUNCH_COPY_QUOTED:
    ...
    bne CRUNCH_REM_COPY_LOOP    ; loop while <> 0 (should never be 0!)
...
CRUNCH_SCAN_KWD_END:
    ...
    bpl CRUNCH_SCAN_KWD_END     ; if not end of keyword go do next

    lda (ut2_pl),Y              ; get byte from keyword table
    bne CRUNCH_TEST_KWD_END     ; go test next word if not zero byte (end of table)
...
; *** end   patch: lower case token recognition V2 ***
    ...
    bpl CRUNCH_RESTORE_IDX      ; branch always (all bytes in buffer are $00-$7F)
...
LAB_SHLN:
    ...
    beq LSRCH_NOT_FOUND         ; pointer was zero so we're done, do 'not found' exit
    ...
    bne LSRCH_HI_DIFF           ; if <> skip low byte check

    lda (Baslnl),Y              ; get line # low byte
    cmp Itempl                  ; compare with temporary integer low byte
LSRCH_HI_DIFF:
    bcs LSRCH_CHK_EQUAL         ; else if temp < this line, exit (passed line#)
...
LSRCH_CHK_EQUAL:
    beq LSRCH_RETURN            ; exit if temp = found line #, carry is set
...
; perform NEW
LAB_NEW:
    bne LSRCH_RETURN            ; exit if not end of statement (to do syntax error)
DO_NEW_CLEAR:
    lda #$00                    ; clear A
    tay                         ; clear Y
...
CLEAR_CMD_BODY:
    ...
    jsr RESTORE_RESET_DPTR      ; perform RESTORE command

; flush stack and clear continue flag
FLUSH_STACK_CLR_CONT:
...
; perform CLEAR
LAB_CLEAR:
    beq CLEAR_CMD_BODY          ; if no following token go do "CLEAR"
...
LAB_LIST:
    bcc LIST_PARSE_START        ; branch if next character numeric (LIST n..)

    beq LIST_PARSE_START        ; branch if next character [NULL] (LIST)

    cmp #token_MINUS            ; compare with token for -
    bne FLUSH_STACK_RTS         ; exit if not - (LIST -m)
...
; (pointer in Baslnl/Baslnh)
    jsr LAB_GBYT                ; scan memory
    beq LIST_CHK_RANGE          ; branch if no more characters

; this bit checks the - is present
    cmp #token_MINUS            ; compare with token for -
    bne LSRCH_RETURN            ; return if not "-" (will be Syntax error)

; LIST [n]-m
; the - was there so set m as the end value
    jsr LAB_IGBY                ; increment and scan memory
    jsr LAB_GFPN                ; get fixed-point number into temp integer
    bne LSRCH_RETURN            ; exit if not ok

LIST_CHK_RANGE:
    lda Itempl                  ; get temporary integer low byte
    ora Itemph                  ; OR temporary integer high byte
    bne LIST_LOOP_PRINT         ; branch if start set
...
LIST_LOOP_PRINT:
    ...
    jsr LAB_CRLF                ; print CR/LF
    lda (Baslnl),Y              ; get next line pointer high byte
; pointer initially set by search at LIST_PARSE_START
    beq LIST_DONE               ; if null all done so exit
    jsr CTRLC_CHECK_VEC         ; do CRTL-C check vector
    ...
    lda (Baslnl),Y              ; get line # high byte
    cmp Itemph                  ; compare with temporary integer high byte
    bne LIST_CMP_LINE_NUM       ; branch if no high byte match

    cpx Itempl                  ; compare with temporary integer low byte
    beq LIST_PRINT_LINE         ; branch if = last line to do (< will pass next branch)

LIST_CMP_LINE_NUM:                   ; else ..
    bcs LIST_DONE               ; if greater all done so exit

LIST_PRINT_LINE:
    sty Tidx1                   ; save index for line
    jsr PRINT_XA_UNSIGNED       ; print XA as unsigned integer
    lda #$20                    ; space is the next character
LIST_PRINT_SPACE_CHR:
    ldy Tidx1                   ; get index for line
    and #$7F                    ; mask top out bit of character
LIST_PRINT_CHAR_LOOP:
    jsr LAB_PRNA                ; go print the character
    cmp #$22                    ; was it " character
    bne LIST_CHK_TOKEN          ; branch if not
...
LIST_CHK_TOKEN:
    ...
    bne LIST_TOKEN_BYTE         ; branch if not [EOL] (go print character)
    ...
    bne LIST_LOOP_PRINT         ; go do next line if not [EOT]
; else ..
LIST_DONE:
    rts

LIST_TOKEN_BYTE:
    bpl LIST_PRINT_CHAR_LOOP    ; just go print it if not token byte

; else was token byte so uncrunch it (maybe)
    bit Oquote                  ; test the open quote flag
    bmi LIST_PRINT_CHAR_LOOP    ; just go print character if open quote set

    ldx #>LAB_KEYT              ; get table address high byte
    asl                         ; *2
    asl                         ; *4
    bcc LIST_TOKEN_CARRY_ADJ    ; branch if no carry

    inx                         ; else increment high byte
    clc                         ; clear carry for add
LIST_TOKEN_CARRY_ADJ:
    adc #<LAB_KEYT              ; add low byte
    bcc LIST_LOAD_TOKEN_ENTRY   ; branch if no carry

    inx                         ; else increment high byte
LIST_LOAD_TOKEN_ENTRY:
    ...
    beq LIST_PRINT_SPACE_CHR    ; if no more characters exit and print
...
LIST_PRINT_KWD_CHARS:
    lda (ut2_pl),Y              ; get character
    dex                         ; decrement character count
    beq LIST_PRINT_SPACE_CHR    ; if last character exit and print

    jsr LAB_PRNA                ; go print the character
    iny                         ; increment index
    bne LIST_PRINT_KWD_CHARS    ; loop for next character
...
LAB_FOR:
    ...
    jsr CHK_STACK_ROOM          ; check room on stack for A bytes
...
; *** end   patch  2.22p5.1   TO expression may get sign bit flipped
    ...
    lda #<FOR_AFTER_TO_PUSH     ; set return address low byte
    ldy #>FOR_AFTER_TO_PUSH     ; set return address high byte
    sta ut1_pl                  ; save return address low byte
    sty ut1_ph                  ; save return address high byte
    jmp PUSH_FAC1_ROUND         ; round FAC1 and put on stack (returns to next instruction)

FOR_AFTER_TO_PUSH:
    lda #<CONST_ONE             ; set 1 pointer low addr (default step size)
    ldy #>CONST_ONE             ; set 1 pointer high addr
    jsr LAB_UFAC                ; unpack memory (AY) into FAC1
    jsr LAB_GBYT                ; scan memory
    cmp #token_STEP             ; compare with STEP token
    bne FOR_AFTER_STEP_EVAL     ; jump if not "STEP"
...
; else do type mismatch
FOR_AFTER_STEP_EVAL:
    jsr GET_FAC1_SIGN           ; return A=FF,C=1/-ve A=01,C=0/+ve
...
; *** end   patch  2.22p5.3   potential return address -$100 (page not incremented) ***
    jsr PUSH_FAC1_SIGN          ; push sign, round FAC1 and put on stack
...
;      BEQ   INTERP_GET_NEXT_BYTE          ; branch if null (immediate mode)

    sta Cpntrl                  ; save continue pointer low byte
    sty Cpntrh                  ; save continue pointer high byte
INTERP_GET_NEXT_BYTE:
    ldy #$00                    ; clear index
    lda (Bpntrl),Y              ; get next byte
    beq INTERP_AT_EOL           ; branch if null [EOL]

    cmp #':'                    ; compare with ":"
    beq INTERP_INCR_AND_SCAN    ; branch if = (statement separator)
...
INTERP_AT_EOL:
    ...
    beq PROG_END_OR_BREAK       ; if null go to immediate mode (was immediate or [EOT] marker)
    ...
    adc Bpntrl                  ; add BASIC execute pointer low byte
    sta Bpntrl                  ; save BASIC execute pointer low byte
    bcc INTERP_INCR_AND_SCAN    ; branch if no overflow

    inc Bpntrh                  ; else increment BASIC execute pointer high byte
INTERP_INCR_AND_SCAN:
    jsr LAB_IGBY                ; increment and scan memory

INTERP_CALL_CODE:
    jsr INTERP_FROM_BPNTRL      ; go interpret BASIC code from (Bpntrl)

INTERP_LOOP_BACK:
    jmp INTERP_INNER_LOOP       ; loop

; interpret BASIC code from (Bpntrl)

INTERP_FROM_BPNTRL:
    beq RESTORE_RTS             ; exit if zero [EOL]

INTERP_DISPATCH:
    asl                         ; *2 bytes per vector and normalise token
    bcs INTERP_LOOKUP_VECTOR    ; branch if was token

    jmp LAB_LET                 ; else go do implied LET

INTERP_LOOKUP_VECTOR:
    cmp #(token_TAB-$80)*2      ; compare normalised token * 2 with TAB
    bcs INTERP_SYNTAX_ERR       ; branch if A>=TAB (do syntax error then warm start)
...
LAB_STOP:
    bcs CTRLC_CHK_STOP          ; branch if token follows STOP
; else just END
; END
...
CTRLC_CHK_STOP:
    bne NULL_SYNTAX_RTS         ; if wasn't CTRL-C or there is a following byte return

    lda Bpntrh                  ; get the BASIC execute pointer high byte
;*** fix p2: skip no longer necessary as the continue pointer is saved anyway
;      EOR   #>Ibuffs          ; compare with buffer address high byte (Cb unchanged)
;      BEQ   CTRLC_DISCARD_RETADDR          ; branch if the BASIC pointer is in the input buffer
...
RESTORE_RESET_DPTR:
    ...
    bcs RESTORE_SAVE_DPTR       ; branch if no underflow
...
LAB_go_search:
    ...
    jmp UNDEF_STMT_ERROR        ; else go do "Undefined statement" error
...
; carry already set for subtract
    ...
    bcs RESTORE_SAVE_DPTR       ; branch if no underflow (save DATA pointer and return)
...
LAB_CONT:
    bne NULL_SYNTAX_RTS         ; if following byte exit to do syntax error

    ldy Cpntrh                  ; get continue pointer high byte
    cpy   #>Ibuffs          ; *** fix p2: test direct mode
    bne CONT_SETUP_RESUME       ; go do continue if we can
...
LAB_RUN:
    bne RUN_N_GOTO              ; branch if RUN n
    jmp RESET_EXEC_CLEAR_VARS   ; reset execution to start, clear variables, flush stack and
; return

; does RUN n

RUN_N_GOTO:
    jsr CLEAR_CMD_BODY          ; go do "CLEAR"
    beq GOSUB_SCAN_GOTO         ; get n and do GOTO n (branch always as CLEAR sets Z=1)
...
LAB_DO:
    ...
    pha                         ; push on stack
    jsr LAB_GBYT                ; scan memory
    jmp INTERP_INNER_LOOP       ; go do interpreter inner loop
...
LAB_GOTO:
    ...
    lda Clineh                  ; get current line high byte
    cmp Itemph                  ; compare with temporary integer high byte
    bcs GOTO_SEARCH_FROM_START  ; branch if >= (start search from beginning)

    tya                         ; else copy line index to A
    sec                         ; set carry (+1)
    adc Bpntrl                  ; add BASIC execute pointer low byte
    ldx Bpntrh                  ; get BASIC execute pointer high byte
    bcc GOTO_SEARCH_FROM_AX     ; branch if no overflow to high byte

    inx                         ; increment high byte
    bcs GOTO_SEARCH_FROM_AX     ; branch always (can never be carry)

; search for line # in temp (Itempl/Itemph) from start of mem pointer (Smeml)

GOTO_SEARCH_FROM_START:
    lda Smeml                   ; get start of mem low byte
    ldx Smemh                   ; get start of mem high byte
...
GOTO_SEARCH_FROM_AX:
    jsr LAB_SHLN                ; search Basic for temp integer line number from AX
    bcc UNDEF_STMT_ERROR        ; if carry clear go do "Undefined statement" error
...
; (stack pointer in X)
    ...
    bne SYNTAX_ERR_JMP          ; if not WHILE go do syntax error and warm start
...
RETURN_NO_GOSUB_ERR:
    ldx #$04                    ; error code $04 ("RETURN without GOSUB" error)
      .byte $2C                 ; makes next line BIT abs (skip next LDX)

UNDEF_STMT_ERROR:                   ; do undefined statement error
...
LAB_RETURN:
    bne GOTO_DONE_RTS           ; exit if following token (to allow syntax error)
...
RETURN_POP_CTX:
    ...
    bne RETURN_NO_GOSUB_ERR     ; branch if no matching GOSUB
...
SET_EXEC_PTR_Y:
    ...
    adc Bpntrl                  ; add BASIC execute pointer low byte
    sta Bpntrl                  ; save BASIC execute pointer low byte
    bcc SET_EXEC_PTR_RTS        ; skip next if no carry
...
SCAN_BYTE_LOOP:
    lda (Bpntrl),Y              ; get next byte
    beq SET_EXEC_PTR_RTS        ; exit if null [EOL]

    cmp Asrch                   ; compare with search character
    beq SET_EXEC_PTR_RTS        ; exit if found

    iny                         ; increment index
    cmp #$22                    ; compare current character with open quote
    bne SCAN_BYTE_LOOP          ; if not open quote go get next character

    beq SCAN_SWAP_CHARS         ; if found go swap search character for alt search character
...
;
;      JMP   INTERP_DISPATCH          ; else it was RETURN so interpret BASIC code from (Bpntrl)
;                              ; but don't return here
;
;LAB_174G
;      JSR   INTERP_FROM_BPNTRL          ; interpret BASIC code from (Bpntrl)
;
...
LAB_no_ELSE:
    jmp INTERP_INNER_LOOP       ; return to the interpreter inner loop
...
ELSE_SCAN_LOOP:
    ...
    lda (Bpntrl),Y              ; get the next BASIC byte
    beq ELSE_FOUND_MATCH        ; if EOL go add the pointer and return

    cmp #token_IF               ; compare the byte with the token for IF
    bne ELSE_CHK_ELSE_TOKEN     ; if not IF token skip the depth increment

    inx                         ; else increment the nesting depth ..
    bne ELSE_SCAN_LOOP          ; .. and continue looking

ELSE_CHK_ELSE_TOKEN:
    cmp #token_ELSE             ; compare the byte with the token for ELSE
    bne ELSE_SCAN_LOOP          ; if not ELSE token continue looking

    dex                         ; was ELSE so decrement the nesting depth
    bne ELSE_SCAN_LOOP          ; loop if still nested
...
ELSE_FOUND_MATCH:
    ...
    bcc ELSE_AFTER_SKIP         ; branch if no overflow to high byte

    inc Bpntrh                  ; else increment the BASIC execute pointer high byte
ELSE_AFTER_SKIP:
    jsr LAB_GBYT                ; scan memory
    bcc IF_RESULT_GOTO_N        ; if numeric do GOTO n
; the code will return to the interpreter loop at the
; tail end of the GOTO <n>

    jmp INTERP_FROM_BPNTRL      ; interpret BASIC code from (Bpntrl)
; the code will return to the interpreter loop at the
...
LAB_REM:
    jsr LAB_SNBL                ; scan for next BASIC line
    jmp SET_EXEC_PTR_Y          ; go set BASIC execute pointer and return, branch always
...
LAB_NONM:
    ...
    cmp #token_GOSUB            ; compare with GOSUB token
    beq ON_DEC_AND_DISPATCH     ; branch if GOSUB

    cmp #token_GOTO             ; compare with GOTO token
ON_BAD_GOTO_GOSUB:
    bne SYNTAX_ERR_ON           ; if not GOTO do syntax error then warm start
...
GFPN_NUMPARSE_LOOP:
    ...
    bcs ON_RTS                  ; return if carry set, end of scan, character was
; not 0-9

    cpx #$19                    ; compare high byte with $19
    tay                         ; ensure Zb = 0 if the branch is taken
    bcs ON_BAD_GOTO_GOSUB       ; branch if >=, makes max line # 63999 because next
...
; and do syntax error
    ...
    bcc GFPN_ADD_DIGIT_OK       ; if no overflow to high byte get next character

    inx                         ; else increment high byte
GFPN_ADD_DIGIT_OK:
    jsr LAB_IGBY                ; increment and scan memory
    jmp GFPN_NUMPARSE_LOOP      ; loop for next character

; perform DEC

LAB_DEC:
    lda #<CONST_NEG_ONE         ; set -1 pointer low byte
      .byte $2C               ; BIT abs to skip the LDA below

; perform INC

LAB_INC:
    lda #<CONST_ONE             ; set 1 pointer low byte
INCDEC_SAVE_PTR:
...
INCDEC_GET_VAR_LOOP:
    ...
    pha                         ; save +/-1 pointer low byte
    ldy #>CONST_ONE             ; set +/-1 pointer high byte (both the same)
    jsr ADD_AY_TO_FAC1          ; add (AY) to FAC1
    jsr LAB_PFAC                ; pack FAC1 into variable (Lvarpl)

    jsr LAB_GBYT                ; scan memory
    cmp #','                    ; compare with ","
    bne ON_PULL_TOKEN_EXIT      ; exit if not "," (either end or error)

; was "," so another INCR variable to do
    jsr LAB_IGBY                ; increment and scan memory
    jmp INCDEC_GET_VAR_LOOP     ; go do next var

IncrErr:
    jmp TYPE_MISMATCH_ERR       ; do "Type mismatch" error then warm start
...
;      BNE   LET_STRING          ; branch if string
; *** with
    jsr LAB_CKTM                ; type match check, keep C (expected type)
    bcs LET_STRING              ; branch if string
; *** end patch
...
LET_STRING:
    ...
    bcc LET_STR_ASSIGN_DESC     ; if less assign value and exit (was in program memory)

    bne LET_STR_NOT_IN_PROG     ; branch if >
...
LET_STR_NOT_IN_PROG:
    ...
    bcc LET_STR_ASSIGN_DESC     ; branch if less (descriptor is on stack)

    bne LET_STR_MAKE_SPACE      ; branch if greater (descriptor is not on stack)

; else high bytes were equal so ..
    lda des_pl                  ; get descriptor pointer low byte
    cmp Svarl                   ; compare start of vars low byte
    bcs LET_STR_MAKE_SPACE      ; branch if >= (descriptor is not on stack)

LET_STR_ASSIGN_DESC:
    lda des_pl                  ; get descriptor pointer low byte
    ldy des_ph                  ; get descriptor pointer high byte
    jmp LET_STR_CLEAN_ASSIGN    ; clean stack, copy descriptor to variable and return

; make space and copy string
LET_STR_MAKE_SPACE:
    ldy #$00                    ; index to length
    lda (des_pl),Y              ; get string length
    jsr COPY_DESC_MAKE_SPACE    ; copy string
    ...
    jsr COPY_STR_FROM_DESC      ; copy string from descriptor (sdescr) to (Sutill)
...
; was numeric get
    ...
    jsr CONV_Y_TO_FAC1          ; convert Y to byte in FAC1
    jmp LAB_PFAC                ; pack FAC1 into variable (Lvarpl) and return
...
; and length on descriptor stack and update stack pointers
    ...
    jmp LET_STRING              ; do string LET and return

; perform PRINT

PRINT_STRING_ITEM:
    jsr PRINT_FROM_DESCRIPTOR   ; print string from Sutill/Sutilh
PRINT_NEXT_TOKEN:
    jsr LAB_GBYT                ; scan memory
...
PRINT_DISPATCH:
    ...
    beq PRINT_DO_TAB_SPC        ; go do TAB/SPC
    cmp #token_SPC              ; compare with SPC( token
    beq PRINT_DO_TAB_SPC        ; go do TAB/SPC
    cmp #','                    ; compare with ","
    beq PRINT_COMMA_TAB         ; go do move to next TAB mark
    cmp #';'                    ; compare with ";"
    beq PRINT_CONTINUE          ; if ";" continue with PRINT processing
    jsr LAB_EVEX                ; evaluate expression
    bit Dtypef                  ; test data type flag, $FF=string, $00=numeric
    bmi PRINT_STRING_ITEM       ; branch if string
    jsr FAC1_TO_STR_STR         ; convert FAC1 to string
    jsr PRINT_QUOTED_STR        ; print " terminated string to Sutill/Sutilh
    ldy #$00                    ; clear index

; don't check fit if terminal width byte is zero

    lda TWidth                  ; get terminal width byte
    beq PRINT_LINE_FIT          ; skip check if zero

    sec                         ; set carry for subtract
    sbc TPos                    ; subtract terminal position
    sbc (des_pl),Y              ; subtract string length
    bcs PRINT_LINE_FIT          ; branch if less than terminal width

    jsr LAB_CRLF                ; else print CR/LF
PRINT_LINE_FIT:
    jsr PRINT_FROM_DESCRIPTOR   ; print string from Sutill/Sutilh
    beq PRINT_NEXT_TOKEN        ; always go continue processing line
...
PRINT_COMMA_TAB:
    ...
    bcc PRINT_CALC_TAB          ; branch if less

    jsr LAB_CRLF                ; else print CR/LF (next line)
    bne PRINT_CONTINUE          ; continue with PRINT processing (branch always)

PRINT_CALC_TAB:
    sec                         ; set carry for subtract
PRINT_TAB_SUBTRACT:
    sbc TabSiz                  ; subtract TAB size
    bcs PRINT_TAB_SUBTRACT      ; loop if result was +ve

    eor #$FF                    ; complement it
    adc #$01                    ; +1 (twos complement)
    bne PRINT_A_SPACES          ; always print A spaces (result is never $00)
...
PRINT_DO_TAB_SPC:
    ...
    bne BAD_INPUT_SYNTAX_ERR    ; if not do syntax error then warm start

    pla                         ; get token back
    cmp #token_TAB              ; was it TAB ?
    bne PRINT_SPC_SPACES        ; if not go do SPC

; calculate TAB offset
    txa                         ; copy integer value to A
    sbc TPos                    ; subtract terminal position
    bcc PRINT_CONTINUE          ; branch if result was < 0 (can't TAB backwards)
...
PRINT_SPC_SPACES:
    ...
    beq PRINT_CONTINUE          ; branch if result was = $0, already here

; print X spaces
PRINT_SPACE_LOOP:
    jsr PRINT_SPACE_CHAR        ; print " "
    dex                         ; decrement count
    bne PRINT_SPACE_LOOP        ; loop if not all done

; continue with PRINT processing
PRINT_CONTINUE:
    jsr LAB_IGBY                ; increment and scan memory
    bne PRINT_DISPATCH          ; if more to print go do it
...
PRINT_FROM_DESCRIPTOR:
    jsr POP_STR_FROM_STK        ; pop string off descriptor stack, or from top of string
; space returns with A = length, X=$71=pointer low byte,
; Y=$72=pointer high byte
    ldy #$00                    ; reset index
    tax                         ; copy length to X
    beq PRNA_RTS                ; exit (RTS) if null string
...
PRINT_STR_LOOP:
    ...
    iny                         ; increment index
    dex                         ; decrement count
    bne PRINT_STR_LOOP          ; loop if not done yet
...
PRINT_SPACE_CHAR:
    lda #' '
      .byte $2C                 ; change next line to BIT abs (skip next LDA)
...
LAB_PRNA:
    cmp #' '                    ; compare with " "
    bcc PRNA_OUTPUT_BYTE        ; branch if less (non printing)
...
PRNA_OUTPUT_BYTE:
    ...
    cmp #$0D                    ; compare with [CR]
    bne PRNA_SET_FLAGS          ; branch if not [CR]

; else print nullct nulls after the [CR]
    stx TempB                   ; save buffer index
    ldx Nullct                  ; get null count
    beq PRNA_CLEAR_TPOS         ; branch if no nulls

    lda #$00                    ; load [NULL]
PRNA_NULL_LOOP:
    jsr LAB_PRNA                ; go print the character
    dex                         ; decrement count
    bne PRNA_NULL_LOOP          ; loop if not all done
...
BAD_INPUT_DATA:
    ...
    bpl INPUT_REDO_MSG          ; branch if INPUT (go do redo)
...
LAB_INPUT:
    ...
    bne INPUT_GET_DATA          ; branch if no prompt string

    jsr PRINT_QUOT_STRING       ; print "..." string
    lda #$3B                    ; load A with ";"
    jsr LAB_SCCA                ; scan for CHR$(A), else do syntax error then warm start
    jsr PRINT_FROM_DESCRIPTOR   ; print string from Sutill/Sutilh
...
READ_NEXT_VAR:
    ...
    sty Bpntrh                  ; set BASIC execute pointer high byte
    jsr LAB_GBYT                ; scan memory
    bne READ_CHK_DTYPE          ; branch if not null

; pointer was to null entry
    bit Imode                   ; test input mode flag, $00=INPUT, $80=READ
    bmi READ_FIND_DATA_STMT     ; branch if READ

; mode was INPUT
    jsr PRINT_QUESTION_CHAR     ; print "?" character (double ? for extended input)
    jsr LAB_INLN                ; print "? " and get BASIC input
    stx Bpntrl                  ; set BASIC execute pointer low byte
    sty Bpntrh                  ; set BASIC execute pointer high byte
READ_SCAN_MEMORY:
...
READ_CHK_DTYPE:
    bit Dtypef                  ; test data type flag, $FF=string, $00=numeric
    bpl READ_NUMERIC_INPUT      ; branch if numeric

; else get string
    sta Srchc                   ; save search character
    cmp #$22                    ; was it " ?
    beq READ_SET_SEARCH_CHR     ; branch if so
...
READ_SET_SEARCH_CHR:
    ...
    adc #$00                    ; c is =1 if we came via the BEQ READ_SET_SEARCH_CHR, else =0
    bcc READ_PRINT_STR          ; branch if no execute pointer low byte rollover

    iny                         ; else increment high byte
READ_PRINT_STR:
    jsr SCAN_SRCHC_STR          ; print Srchc or Asrch terminated string to Sutill/Sutilh
    jsr RESTORE_BPNTR_TEMP      ; restore BASIC execute pointer from temp (Btmpl/Btmph)
    jsr LET_STRING              ; go do string LET
    jmp READ_CHK_SEPARATOR      ; go check string terminator

; get numeric INPUT
READ_NUMERIC_INPUT:
    jsr STR_TO_FAC1             ; get FAC1 from string
    jsr LAB_PFAC                ; pack FAC1 into (Lvarpl)
READ_CHK_SEPARATOR:
    jsr LAB_GBYT                ; scan memory
    beq READ_RESTORE_PTRS       ; branch if null (last entry)

    cmp #','                    ; else compare with ","
    beq READ_NEXT_ITEM          ; branch if ","

    jmp BAD_INPUT_DATA          ; else go handle bad input data
...
READ_RESTORE_PTRS:
    ...
    sty Bpntrh                  ; set BASIC execute pointer high byte
    jsr LAB_GBYT                ; scan memory
    beq INPUT_READ_END          ; if null go do extra ignored message

    jsr SCAN_COMMA              ; else scan for "," , else do syntax error then warm start
    jmp READ_NEXT_VAR           ; go INPUT next variable from list
...
READ_CHK_DATA_TOKEN:
    ...
    iny                         ; increment index
    tax                         ; copy to X
    jsr SET_EXEC_PTR_Y          ; set BASIC execute pointer
    cpx #token_DATA             ; compare with "DATA" token
    beq READ_SCAN_MEMORY        ; was "DATA" so go do next READ

    bne READ_FIND_DATA_STMT     ; go find next statement if not "DATA"
...
INPUT_READ_END:
    ...
    bpl INPUT_CHK_END           ; branch if INPUT

    jmp RESTORE_SAVE_DPTR       ; save AY as DATA pointer and return

; we were getting INPUT
INPUT_CHK_END:
    ldy #$00                    ; clear index
    lda (Rdptrl),Y              ; get next byte
    bne INPUT_EXTRA_IGNORED     ; error if not end of INPUT
...
INPUT_EXTRA_IGNORED:
    ...
    jmp PRINT_STR_FROM_AY       ; print null terminated string from memory and return
...
SEARCH_FOR_LOOP:
    ...
    bne SEARCH_FOR_RTS          ; exit if not FOR token

; was FOR token
    lda Frnxth                  ; get var pointer for FOR/NEXT high byte
    bne SEARCH_FOR_CHK_MATCH    ; branch if not null
...
SEARCH_FOR_ADVANCE:
    ...
    bne SEARCH_FOR_LOOP         ; loop if not at start of stack
...
LAB_NEXT:
    bne NEXT_VAR                ; branch if NEXT var

    ldy #$00                    ; else clear Y
    beq NEXT_SET_FRNXT          ; branch always (no variable to search for)
...
; (both cleared if no variable defined)
    jsr SEARCH_FOR_ON_STACK     ; search the stack for FOR activity
    beq NEXT_SET_STACK          ; branch if found

    ldx #$00                    ; else set error $00 ("NEXT without FOR" error)
NEXT_ERR_OR_OOM:
    beq TYPE_DO_ERROR           ; do error #X, then warm start
...
NEXT_SET_STACK:
    ...
    jsr ADD_AY_TO_FAC1          ; add (FOR variable) to FAC1
    jsr LAB_PFAC                ; pack FAC1 into (FOR variable)
    ldy #>LAB_STAK              ; point to stack page high byte
    jsr CMP_FAC1_AY_PTRSAVED    ; compare FAC1 with (Y,ut2_pl) (TO value)
    tsx                         ; get stack pointer back
    cmp LAB_STAK+8,X            ; compare step sign
    beq NEXT_LOOP_DONE          ; branch if = (loop complete)
...
NEXT_LOOP_DONE:
    ...
    jsr LAB_GBYT                ; scan memory
    cmp #','                    ; compare with ","
    bne NEXT_LOOP_AGAIN         ; branch if not "," (go do interpreter inner loop)

; was "," so another NEXT variable to do
    jsr LAB_IGBY                ; else increment and scan memory
    jsr NEXT_VAR                ; do NEXT (var)
...
LAB_CKTM:
    bit Dtypef                  ; test data type flag, $FF=string, $00=numeric
    bmi TYPE_WAS_STRING         ; branch if data type is string
; else data type was numeric
    bcs TYPE_MISMATCH_ERR       ; if required type is string do type mismatch error
TYPE_OK_RTS:
    rts
; data type was string, now check required type
TYPE_WAS_STRING:
    bcs TYPE_OK_RTS             ; exit if required type is string
...
LAB_EVEX:
    ...
    bne EVEX_DEC_BPNTRL         ; skip next if not zero
...
EVAL_CHK_OPER_TOKEN:
    ...
    bcc EVAL_TOKEN_OUT_RANGE    ; branch if < token_GT

    cmp #$03                    ; compare with ">" to "<" tokens
    bcs EVAL_TOKEN_OUT_RANGE    ; branch if >= token_SGN (highest evaluation function +1)
...
; (A = 1, 2 or 4. 1 if >, 2 if =, 4 if <)
    ...
    bcc EVAL_SYNTAX_ERR         ; if <(comp_f) do syntax error then warm start
; was more than one <, = or >)

    sta comp_f                  ; save new compare function flag
    jsr LAB_IGBY                ; increment and scan memory
    jmp EVAL_CHK_OPER_TOKEN     ; go do next character

; token is < ">" or > "<" tokens
EVAL_TOKEN_OUT_RANGE:
    ldx comp_f                  ; get compare function flag
    bne EVAL_DO_COMPARE         ; branch if compare function

    bcs EVAL_DO_FUNCTION        ; go do functions

; else was <  token_GT so is operator or lower
    adc #token_GT-token_PLUS    ; add # of operators (+, -, *, /, ^, AND, OR or EOR)
    bcc EVAL_DO_FUNCTION        ; branch if < + operator

; carry was set so token was +, -, *, /, ^, AND, OR or EOR
    bne EVAL_CALC_OPER_IDX      ; branch if not + token

    bit Dtypef                  ; test data type flag, $FF=string, $00=numeric
    bpl EVAL_CALC_OPER_IDX      ; branch if not string

; will only be $00 if type is string and token was +
    jmp CONCAT_STRINGS          ; add strings, string 1 is in descriptor des_pl, string 2
...
EVAL_POP_PREV_PREC:
    ...
    bcs EVAL_CHK_PREC_64        ; branch if A >=

    jsr LAB_CTNM                ; check if source is numeric, else do type mismatch
EVAL_PUSH_PREC_DO:
    pha                         ; save precedence
EVAL_EXEC_FUNC:
    jsr EVAL_GET_VEC_EXEC       ; get vector, execute function then continue evaluation
    pla                         ; restore precedence
    ldy prstk                   ; get precedence stacked flag
    bpl EVAL_CMP_STACKED_PREC   ; branch if stacked values

    tax                         ; copy precedence (set flags)
    beq EVAL_GET_FAC1_EXP       ; exit if done

    bne EVAL_POP_FAC2           ; else pop FAC2 and return, branch always
...
EVAL_DO_COMPARE:
    ...
    ldx Bpntrl                  ; get BASIC execute pointer low byte
    bne EVAL_DEC_BPNTRL         ; branch if no underflow
...
token_LT_PLUS  = token_LT-token_PLUS
    ...
    sta comp_f                  ; save new compare function flag
    bne EVAL_POP_PREV_PREC      ; branch always

EVAL_CMP_STACKED_PREC:
    cmp LAB_OPPT,Y              ;.compare with stacked function precedence
    bcs EVAL_POP_FAC2           ; branch if A >=, pop FAC2 and return

    bcc EVAL_PUSH_PREC_DO       ; branch always
...
;   .ENDIF
; *** end   patch  2.22p5.3   potential return address -$100 (page not incremented) ***
    jsr PUSH_FAC1_SIGN          ; function will return here, then the next RTS will call the function
    lda comp_f                  ; get compare function flag
    pha                         ; push compare evaluation byte
    lda LAB_OPPT,Y              ; get precedence byte
    jmp EVAL_PUSH_PREC          ; continue evaluating expression
...
EVAL_CHK_PREC_64:
    ...
    beq EVAL_SAVE_PRSTK         ; branch if was $64 (< function)
...
PRINT_QUOT_STRING:
    ...
    bcc PRINT_QUOT_SETUP        ; branch if no overflow

    iny                         ; increment high byte
PRINT_QUOT_SETUP:
    jsr PRINT_QUOTED_STR        ; print " terminated string to Sutill/Sutilh
    jmp RESTORE_BPNTR_TEMP      ; restore BASIC execute pointer from temp and return

; get value from line

LAB_GVAL:
    jsr LAB_IGBY                ; increment and scan memory
    bcs GVAL_NOT_NUMERIC        ; branch if not numeric character

; else numeric string found (e.g. 123)
GVAL_NUMERIC_STR:
    jmp STR_TO_FAC1             ; get FAC1 from string and return
...
GVAL_NOT_NUMERIC:
    tax                         ; set the flags
    bmi GVAL_DO_TOKEN           ; if -ve go test token values

; else it is either a string, number, variable or (<expr>)
    cmp #'$'                    ; compare with "$"
    beq GVAL_NUMERIC_STR        ; branch if "$", hex number

    cmp #'%'                    ; else compare with "%"
    beq GVAL_NUMERIC_STR        ; branch if "%", binary number

    cmp #'.'                    ; compare with "."
    beq GVAL_NUMERIC_STR        ; if so get FAC1 from string and return (e.g. was .123)

; it wasn't any sort of number so ..
    cmp #$22                    ; compare with "
    beq PRINT_QUOT_STRING       ; branch if open quote
...
; evaluate expression within parentheses
    ...
    cmp #'('                    ; compare with "("
    bne GET_VAR_TO_FAC1         ; if not "(" get (var), return value in FAC1 and $ flag
...
GVAL_DO_TOKEN:
    cmp #token_MINUS            ; compare with token for -
    beq FUNC_SETUP_NEG          ; branch if - token (do set-up for functions)
...
; wasn't -n so ..
    ...
    bne GVAL_CHK_FN_TOKEN       ; branch if not token for NOT

; was NOT token
token_EQUAL_PLUS     = token_EQUAL-token_PLUS
    ldy #token_EQUAL_PLUS*3     ; offset to NOT function
    bne FUNC_SETUP_EXEC         ; do set-up for function then execute (branch always)
...
GVAL_CHK_FN_TOKEN:
    ...
    bne GVAL_CHK_SGN_RANGE      ; branch if not token for FN

    jmp FN_EVALUATE             ; go evaluate FNx
...
GVAL_CHK_SGN_RANGE:
    ...
    bcs FUNC_SETUP_VECTOR       ; if a function token go do it
...
;      JMP   EVAL_EXEC_FUNC          ; execute function then continue evaluation
; *** with
...
LAB_1C13b:
    jmp EVAL_EXEC_FUNC          ; execute function then continue evaluation
LAB_1C13a:
    jmp TYPE_MISMATCH_ERR       ; throw "type mismatch error" then warm start
...
GET_VAR_TO_FAC1:
    ...
    ldx Dtypef                  ; get data type flag, $FF=string, $00=numeric
    bmi GET_VAR_STRING_RTS      ; if string then return (does RTS)
...
FUNC_SETUP_VECTOR:
    ...
    beq FUNC_DO_CALL            ; skip pre process if null vector
...
LAB_PPFS:
    jsr GVAL_EVAL_IN_PARENS     ; process expression in parenthesis
...
LAB_LRMS:
    ...
    jsr SCAN_COMMA              ; scan for ",", else do syntax error then warm start
...
LAB_BHSS:
    ...
    jsr FLOAT_TO_FIXED          ; convert FAC1 floating-to-fixed
...
LAB_CFAC:
    ...
    beq BHSS_DO_FUNCTION        ; if ")" go do rest of function
...
GetFirst:
    ...
    lda FAC1_3                  ; get FAC1 mantissa3
    sta XOAw_l                  ; save it
    jsr COPY_FAC2_TO_FAC1       ; copy FAC2 to FAC1 (get 2nd value in expression)
    jsr LAB_EVIR                ; evaluate integer expression (no sign check)
    lda FAC1_3                  ; get FAC1 mantissa3
GETFIRST_RTS:
    rts
...
LAB_LTHAN:
    jsr LAB_CKTM                ; type match check, set C for string
    bcs STR_COMPARE             ; branch if string
...
; do numeric < compare
    ...
    jsr CMP_FAC1_AY             ; compare FAC1 with FAC2 (AY)
    tax                         ; copy result
    jmp STR_CMP_EVAL            ; go evaluate result
...
; Y=pointer high byte
    ...
    jsr POP_DESC_YA             ; pop (YA) descriptor off stack or from top of string space
...
; Y=pointer high byte
    ...
    beq STR_CMP_LEN_DONE        ; branch if str 1 length = string 2 length

    lda #$01                    ; set str 1 length > string 2 length
    bcc STR_CMP_LEN_DONE        ; branch if so
...
STR_CMP_BYTE_LOOP:
    iny                         ; increment index
    dex                         ; decrement count
    bne STR_CMP_NEXT_BYTE       ; branch if still bytes to do

    ldx FAC1_s                  ; get length compare back
STR_CMP_EVAL:
    bmi STR_CMP_RESULT          ; branch if str 1 < str 2

    clc                         ; flag str 1 <= str 2
    bcc STR_CMP_RESULT          ; go evaluate result

STR_CMP_NEXT_BYTE:
    lda (FAC2_2),Y              ; get string 2 byte
    cmp (FAC1_1),Y              ; compare with string 1 byte
    beq STR_CMP_BYTE_LOOP       ; loop if bytes =

    ldx #$FF                    ; set str 1 < string 2
    bcs STR_CMP_RESULT          ; branch if so
...
STR_CMP_RESULT:
    ...
    beq STR_CMP_SAVE_RESULT     ; branch if 0 (compare is false)

    lda #$FF                    ; else set result true
STR_CMP_SAVE_RESULT:
    jmp SAVE_A_AS_INT           ; save A as integer byte and return
...
LAB_DIM:
    ...
    jsr VAR_SEARCH_INIT         ; search for variable
    jsr LAB_GBYT                ; scan memory
    bne DIM_SCAN_COMMA          ; scan for "," and loop if not null
...
VAR_SAVE_NAME:
    ...
    bcs VAR_VALID_FIRST         ; branch if ok
...
VAR_VALID_FIRST:
    ...
    bcc VAR_COPY_2ND_CHR        ; branch if character = "0"-"9" (ok)

; 2nd character wasn't "0" to "9" so ..
    jsr LAB_CASC                ; check byte, return C=0 if<"A" or >"Z"
    bcc VAR_CHK_STRING_TYPE     ; branch if <"A" or >"Z" (go check if string)
...
VAR_SKIP_EXTRA_CHR:
    ...
    bcc VAR_SKIP_EXTRA_CHR      ; loop if character = "0"-"9" (ignore)

    jsr LAB_CASC                ; check byte, return C=0 if<"A" or >"Z"
    bcs VAR_SKIP_EXTRA_CHR      ; loop if character = "A"-"Z" (ignore)

; check if string variable
VAR_CHK_STRING_TYPE:
    cmp #'$'                    ; compare with "$"
    bne VAR_CHECK_SUBSCRIPT     ; branch if not string

; to introduce a new variable type (% suffix for integers say) then this branch
; will need to go to that check and then that branch, if it fails, go to VAR_CHECK_SUBSCRIPT
; type is string
...
VAR_CHECK_SUBSCRIPT:                   ; gets here with character after var name in A
    stx Varnm2                  ; save 2nd character
    ora Sufnxf                  ; or with subscript/FNX flag (or FN name)
    cmp #'('                    ; compare with "("
    bne VAR_PLAIN_VAR_LOOKUP    ; branch if not "("

    jmp ARRAY_FIND_OR_MAKE      ; go find, or make, array
...
; 1st characters match so compare 2nd characters
    ...
    beq VAR_FOUND_MATCH         ; branch if match (found var)
...
VAR_ADVANCE_NEXT:
    ...
    bcc VAR_SEARCH_SAVE_LO      ; loop if no overflow to high byte

    inx                         ; else increment high byte
    bne VAR_SEARCH_SAVE_HI      ; loop always (RAM doesn't extend to $FFFF !)

; check byte, return C=0 if<"A" or >"Z" or "a" to "z"

LAB_CASC:
    cmp #'a'                    ; compare with "a"
    bcs VAR_CHK_ALPHA_AZ2       ; go check <"z"+1

; check byte, return C=0 if<"A" or >"Z"

VAR_CHK_ALPHA_AZ:
    cmp #'A'                    ; compare with "A"
    bcc VAR_ALPHA_CHK_RTS       ; exit if less
...
GET_VAR_CALLER_RET2 = GET_VAR_TO_FAC1+2
    cmp #<GET_VAR_CALLER_RET2   ; compare with expected calling routine return low byte
    bne VAR_CREATE_NEW          ; if not get (var) go create new var

; This will only drop through if the call was from GET_VAR_TO_FAC1 and is only called
...
; else return dummy null value
    lda #<CONST_NULL_WORD       ; low byte point to $00,$00
; (uses part of misc constants table)
    ldy #>CONST_NULL_WORD       ; high byte point to $00,$00
    rts
...
VAR_CREATE_NEW:
    ...
    bcc VAR_SET_NEW_BLOCK       ; branch if no overflow to high byte
...
VAR_FOUND_MATCH:
    ...
    bcc VAR_RETURN_ADDR         ; branch if no overflow from add
...
ARRAY_SETUP_PTR:
    ...
    bcc ARRAY_SAVE_DATAPTR      ; branch if no overflow
...
LAB_EVPI:
    lda FAC1_s                  ; get FAC1 sign (b7)
    bmi EVIR_NEG_ERR            ; do function call error if -ve
...
LAB_EVIR:
    ...
    bcc EVIR_FLOAT_TO_FIXED     ; branch if n<2^16 (is ok)

    lda #<CONST_NEG32768        ; set pointer low byte to -32768
    ldy #>CONST_NEG32768        ; set pointer high byte to -32768
    jsr CMP_FAC1_AY             ; compare FAC1 with (AY)
EVIR_NEG_ERR:
    bne LAB_FCER                ; if <> do function call error then warm start

EVIR_FLOAT_TO_FIXED:
    jmp FLOAT_TO_FIXED          ; convert FAC1 floating-to-fixed and return
...
ARRAY_GET_DIMS:
    ...
    jsr LAB_GBYT                ; scan memory
    cmp #','                    ; compare with ","
    beq ARRAY_GET_DIMS          ; if found go do next dimension

    sty Dimcnt                  ; store dimensions count
    jsr SCAN_CLOSE_PAREN        ; scan for ")" , else do syntax error then warm start
...
ARRAY_CHK_END:
    ...
    bne ARRAY_SEARCH            ; branch if not reached array mem end

    cpx Earryl                  ; else compare with array mem end low byte
    beq ARRAY_BUILD             ; go build array if not found
...
ARRAY_NO_MATCH:
    ...
    bcc ARRAY_CHK_END           ; if no overflow go check next array

; do array bounds error

ARRAY_BOUNDS_ERR:
    ldx #$10                    ; error code $10 ("Array bounds" error)
      .byte $2C                 ; makes next bit BIT abs (skip next LDX)
...
ARRAY_FOUND:
    ...
    lda Defdim                  ; get DIM flag
    bne ARRAY_DO_ERROR          ; if we are trying to dimension it do error #X, then warm
; start

; found the array and we're not dimensioning it so we must find an element in it

    jsr ARRAY_SETUP_PTR         ; set-up array pointer (Adatal/h) to first element in array
; (Astrtl,Astrth points to start of array)
    ...
    bne ARRAY_BOUNDS_ERR        ; if wrong do array bounds error, could do "Wrong
                                ; dimensions" error here .. if we want a different
                                ; error message

    jmp ARRAY_FIND_ELEMENT      ; found array so go get element
                                ; (could jump to ARRAY_FIND_ELEMENT as all ARRAY_GET_DIM_CNT does is take
...
ARRAY_CALC_SIZE:
    ...
    bvc ARRAY_SAVE_DIM          ; branch if b6 of Defdim is clear
...
ARRAY_SAVE_DIM:
    ...
    jsr ARRAY_DIM_MULTIPLY      ; does XY = (Astrtl),Y * (Asptl)
    ...
    bne ARRAY_CALC_SIZE         ; loop while not = 0

    adc Adatah                  ; add size high byte to first element high byte
; (carry is always clear here)
    bcs ARRAY_OOM_JMP           ; if overflow go do "Out of memory" error
    ...
    bcc ARRAY_ZERO_SETUP        ; branch if no carry

    iny                         ; else increment end of array high byte
    beq ARRAY_OOM_JMP           ; if overflow go do "Out of memory" error
...
ARRAY_ZERO_NEXT_BLK:
    ...
    bne ARRAY_ELEM_RTS          ; exit (RET) if this was a DIM command
...
ARRAY_ELEM_LOOP:
    ...
    bcc ARRAY_BOUNDS_INC_IDX    ; branch if within bounds

    bne ARRAY_BOUNDS_JMP        ; if outside bounds do array bounds error
...
; else high byte was = so test low bytes
    ...
    bcc ARRAY_BOUNDS_OK         ; branch if within bounds

ARRAY_BOUNDS_JMP:
    jmp ARRAY_BOUNDS_ERR        ; else do array bounds error

ARRAY_OOM_JMP:
    jmp LAB_OMER                ; do "Out of memory" error then warm start

ARRAY_BOUNDS_INC_IDX:
    iny                         ; index to array bound low byte
ARRAY_BOUNDS_OK:
    lda Aspth                   ; get array data pointer high byte
    ora Asptl                   ; OR with array data pointer low byte
    beq ARRAY_SKIP_MULTIPLY     ; branch if array data pointer = null (skip multiply)
...
ARRAY_SKIP_MULTIPLY:
    ...
    dec Dimcnt                  ; decrement dimensions count
    bne ARRAY_ELEM_LOOP         ; loop if dimensions still to do
...
ARRAY_MUL_LOOP:
    ...
    bcc ARRAY_MUL_NEXT_BIT      ; skip add if no carry
...
ARRAY_MUL_NEXT_BIT:
    ...
    bne ARRAY_MUL_LOOP          ; loop until all done
...
LAB_FRE:
    lda Dtypef                  ; get data type flag, $FF=string, $00=numeric
    bpl FRE_NUMERIC             ; branch if numeric
...
LAB_AYFC:
    ...
    jmp SET_EXP_NORM            ; set exp=X, clear FAC1_3, normalise and return
...
LAB_CKRN:
    ...
    inx                         ; increment it
    bne ARRAY_ELEM_RTS          ; return if can continue not direct mode
...
; perform DEF
LAB_DEF:
    jsr FN_CHK_SYNTAX           ; check FNx syntax
    sta func_l                  ; save function pointer low byte
    sty func_h                  ; save function pointer high byte
    jsr LAB_CKRN                ; check not Direct (back here if ok)
    jsr SCAN_OPEN_PAREN         ; scan for "(" , else do syntax error then warm start
    ...
    jmp FN_SAVE_EXEC_PTR        ; put execute pointer and variable pointer into function
; and return
...
; return character after A
    ...
    jsr VAR_SAVE_NAME           ; search for FN variable
...
FN_EVALUATE:
    ...
    beq DO_ERR_WARM             ; if zero go do undefined function error
...
FN_STACK_VAR:
    ...
    pha                         ; stack it
    dey                         ; decrement index
    bpl FN_STACK_VAR            ; loop until variable stacked

    ldy Cvarah                  ; get variable address high byte
    jsr PACK_FAC1_XY            ; pack FAC1 (function expression value) into (XY)
...
; else do type mismatch
    ...
    jsr LAB_GBYT                ; scan memory
    beq FN_RESTORE_PTRS         ; branch if null (should be [EOL] marker)
...
LAB_CSTRS:
    jsr LAB_CTNM                ; check if source is numeric, else do type mismatch
    jsr FAC1_TO_STR_CSTR        ; convert FAC1 to string
    lda #<Decssp1               ; set result string low pointer
    ldy #>Decssp1               ; set result string high pointer
    bra PRINT_QUOTED_STR        ; print null terminated string to Sutill/Sutilh
...
LAB_STRS:
    ...
    lda #<Decssp1               ; set result string low pointer
    ldy #>Decssp1               ; set result string high pointer
    bra PRINT_QUOTED_STR        ; print null terminated string to Sutill/Sutilh (beq?)

; Do string vector
; copy des_pl/h to des_2l/h and make string space A bytes long

COPY_DESC_MAKE_SPACE:
...
LAB_MSSP:
    jsr MAKE_STR_SPACE_INIT     ; make space in string memory for string A long
...
SCAN_STR_LOOP:
    ...
    beq SCAN_STR_CLEAR_CARRY    ; exit loop if null byte [EOS]

    cmp Srchc                   ; compare with search character (terminator 1)
    beq SCAN_STR_TERMINATOR     ; branch if terminator

    cmp Asrch                   ; compare with terminator 2
    bne SCAN_STR_LOOP           ; loop if not terminator 2

SCAN_STR_TERMINATOR:
    cmp #$22                    ; compare with "
    beq SCAN_STR_SAVE_LEN       ; branch if " (carry set if = !)

SCAN_STR_CLEAR_CARRY:
...
SCAN_STR_SAVE_LEN:
    ...
    bcc SCAN_STR_SAVE_END       ; branch if no low byte overflow
...
; string in utility area, move to string memory
    tya                         ; copy length to A
    jsr COPY_DESC_MAKE_SPACE    ; copy des_pl/h to des_2l/h and make string space A bytes
; long
    ldx ssptr_l                 ; get string start low byte
    ldy ssptr_h                 ; get string start high byte
    jsr STORE_STR_FROM_XY       ; store string A bytes long from XY to (Sutill)
...
LAB_RTST:
    ...
    bne PUT_STR_ON_DESC_STK     ; branch if space on string stack
...
MAKE_STR_SPACE:
    ...
    bcs MAKE_STR_CHK_LO         ; skip decrement if no underflow

    dey                         ; decrement bottom of string space high byte
MAKE_STR_CHK_LO:
    cpy Earryh                  ; compare with array mem end high byte
    bcc MAKE_STR_OOM            ; do out of memory error if less

    bne MAKE_STR_OK             ; if not = skip next test
...
MAKE_STR_OOM:
    ...
    bmi STR_TOO_COMPLEX_ERR     ; if set then do error code X

    jsr LAB_GARB                ; else go do garbage collection
    lda #$80                    ; flag for garbage collected
    sta Gclctd                  ; set garbage collected flag
    pla                         ; pull length
    bne MAKE_STR_SPACE          ; go try again (loop always, length should never be = $00)
...
GARB_DESC_STK_LOOP:
    ...
    beq GARB_DO_STR_VARS        ; branch if =

    jsr GARB_CHECK_STR_LEN      ; go garbage collect descriptor stack
    beq GARB_DESC_STK_LOOP      ; loop always
...
GARB_STR_VAR_LOOP:
    ...
    bne GARB_COLLECT_STR        ; branch if no high byte match

    cmp Sarryl                  ; else compare start of arrays low byte
    beq GARB_DO_ARRAYS          ; branch if = var mem end

GARB_COLLECT_STR:
    jsr GARB_CHK_STR_VAR        ; go garbage collect strings
    beq GARB_STR_VAR_LOOP       ; loop always
...
GARB_CHK_ARR_END:
    ...
    bne GARB_PROCESS_ARRAY      ; branch if not at end

    cmp Earryl                  ; else compare with array mem end low byte
    beq GARB_MOVE_OR_EXIT       ; tidy up and exit if at end
...
GARB_PROCESS_ARRAY:
    ...
    bpl GARB_ARRAY_LOOP         ; skip if not string array
...
; was string array so ..
    ...
    jsr GARB_ADD_STEP           ; go set up for first element
GARB_ARR_ELEM_LOOP:
    cpx Nbendh                  ; compare with start of next array high byte
    bne GARB_DEFRAG_ARR_STR     ; branch if <> (go do this array)

    cmp Nbendl                  ; else compare element pointer low byte with next array
; low byte
    beq GARB_CHK_ARR_END        ; if equal then go do next array

GARB_DEFRAG_ARR_STR:
    jsr GARB_CHECK_STR_LEN      ; go defrag array strings
    beq GARB_ARR_ELEM_LOOP      ; go do next array string (loop always)
...
GARB_CHK_STR_VAR:
    ...
    bpl GARB_ADVANCE_PTR        ; if not string, step pointer to next var and return

    iny                         ; else increment index
GARB_CHECK_STR_LEN:
    lda (ut1_pl),Y              ; get string length
    beq GARB_ADVANCE_PTR        ; if null, step pointer to next string and return
    ...
    cmp Sstorh                  ; compare bottom of string space high byte
    bcc GARB_STR_IN_RANGE       ; branch if less

    bne GARB_ADVANCE_PTR        ; if greater, step pointer to next string and return

; high bytes were = so compare low bytes
    cpx Sstorl                  ; compare bottom of string space low byte
    bcs GARB_ADVANCE_PTR        ; if >=, step pointer to next string and return

; string pointer is < string storage pointer (pos in mem)
GARB_STR_IN_RANGE:
    cmp Histrh                  ; compare to highest string high byte
    bcc GARB_LOAD_STEP          ; if <, step pointer to next string and return

    bne GARB_UPDATE_HIGHEST     ; if > update pointers, step to next and return
...
GARB_ADD_STEP:
    ...
    sta ut1_pl                  ; save pointer low byte
    bcc GARB_RTS                ; branch if no overflow
...
; *** end   patch  2.22p5.5  garbage collection may overlap temporary strings
    beq GARB_RTS                ; exit if nothing to move
    ...
    jsr OPEN_MEM_NO_EARRY       ; open up space in memory, don't set array end
    ...
    jmp GARB_INIT               ; re-run routine from last ending
...
; *** end patch    2.22p5.4  concatenate MINUS or NOT() crashes EhBASIC  ***
    ...
    bcc CONCAT_MAKE_SPACE       ; branch if no overflow
...
; ut1_ph = pointer high byte
    jsr STORE_STR_FROM_UTPL     ; store string A bytes long from (ut1_pl) to (Sutill)
...
; Y=ut1_ph=pointer high byte
    jsr LAB_RTST                ; check for space on descriptor stack then put string
                                ...
                                ; pointers
    jmp EVAL_GET_NEXT_OPER      ;.continue evaluation
...
STORE_STR_FROM_UTPL:
    ...
    beq STORE_STR_RTS           ; branch if = $0 (null string) no need to add zero length
...
STORE_STR_LOOP:
    ...
    bne STORE_STR_LOOP          ; loop while <> 0

    tya                         ; restore length from Y
STORE_STR_ADV_SUTILL:
    clc                         ; clear carry for add
    adc Sutill                  ; add string utility ptr low byte
    sta Sutill                  ; save string utility ptr low byte
    bcc STORE_STR_RTS           ; branch if no carry
...
POP_DESC_YA:
    ...
    pla                         ; get string length back
    plp                         ; restore status
    bne POP_DESC_SAVE_PTR       ; branch if pointer <> last_sl,last_sh

    cpy Sstorh                  ; compare bottom of string space high byte
    bne POP_DESC_SAVE_PTR       ; branch if <>
    ...
    sta Sstorl                  ; save bottom of string space low byte
    bcc POP_DESC_RESTORE_LEN    ; skip increment if no overflow

    inc Sstorh                  ; increment bottom of string space high byte
POP_DESC_RESTORE_LEN:
...
LAB_LEFT:
    ...
    jsr STRFUNC_PULL_PARAMS     ; pull string data and byte parameter from stack
; return pointer in des_2l/h, byte in A (and X), Y=0
    cmp (des_2l),Y              ; compare byte parameter with string length
    tya                         ; clear A
    beq STRFUNC_DO_COPY         ; go do string copy (branch always)
...
STRFUNC_DO_COPY:
    bcc STRFUNC_SAVE_OFFSET     ; branch if string length > byte parameter
...
; Y=ut1_ph=pointer high byte
    ...
    bcc STRFUNC_ADJ_PTR         ; branch if no overflow
...
LAB_MIDS:
    ...
    jsr LAB_GBYT                ; scan memory
    cmp #')'                    ; compare with ")"
    beq MIDS_NO_SECOND_PARAM    ; branch if = ")" (skip second byte get)

    jsr SCAN_COMMA              ; scan for "," , else do syntax error then warm start
    jsr LAB_GTBY                ; get byte parameter (use copy in mids_l)
MIDS_NO_SECOND_PARAM:
    jsr STRFUNC_PULL_PARAMS     ; pull string data and byte parameter from stack
 ; return pointer in des_2l/h, byte in A (and X), Y=0
    ...
    sbc (des_2l),Y              ; subtract string length
    bcs STRFUNC_COPY_LEN        ; if start>string length go do null string

    eor #$FF                    ; complement -length
    cmp mids_l                  ; compare byte parameter
    bcc STRFUNC_PUSH_LEN        ; if length>remaining string go do RIGHT$

    lda mids_l                  ; get length byte
    bcs STRFUNC_PUSH_LEN        ; go do string copy (branch always)
...
STRFUNC_PULL_PARAMS:
    ...
    beq FUNC_CALL_ERR_JMP       ; if null do function call error then warm start
...
LC_loop:
    ...
    jsr VAR_CHK_ALPHA_AZ        ; is character "A" to "Z"
...
LAB_LENS:
    ...
    jmp CONV_Y_TO_FAC1          ; convert Y to byte in FAC1 and return
...
LAB_EVBY:
    ...
    bne FUNC_CALL_ERR_JMP       ; if top byte <> 0 do function call error then warm start
...
LAB_VAL:
    jsr LAB_ESGL                ; evaluate string, get length in A (and Y)
    bne VAL_PROCESS_STR         ; branch if not null string

; string was null so set result = $00
    jmp CLEAR_FAC1_EXP          ; clear FAC1 exponent and sign and return
...
; *** with
    ...
    pla                         ; get length back
    jsr STORE_STR_FROM_UTPL     ; copy string (ut1_pl) -> (Sutill) for A bytes
...
LAB_F2FX:
    lda FAC1_e                  ; get FAC1 exponent
    cmp #$98                    ; compare with exponent = 2^24
    bcs FUNC_CALL_ERR_JMP       ; if >= do function call error then warm start
...
LAB_WAIT:
    ...
    ldx #$00                    ; clear mask
    jsr LAB_GBYT                ; scan memory
    beq WAIT_NO_3RD_ARG         ; skip if no third argument
...
WAIT_LOOP:
    ...
    beq WAIT_LOOP               ; loop if result is zero
...
SUBTRACT_FROM_AY:
    jsr UNPACK_AY_TO_FAC2       ; unpack memory (AY) into FAC2
...
ADD_SHIFT_FACX:
    jsr SHIFT_FACX_RIGHT        ; shift FACX A times right (>8 shifts)
    bcc ADD_DO_MANTISSAS        ;.go subtract mantissas

; add 0.5 to FAC1

ADD_HALF_TO_FAC1:
    lda #<CONST_HALF            ; set 0.5 pointer low byte
    ldy #>CONST_HALF            ; set 0.5 pointer high byte
...
LAB_ADD:
    bne ADD_FAC1_NONZERO        ; branch if FAC1 was not zero
...
COPY_FAC2_LOOP:
    ...
    dex                         ; decrement count
    bne COPY_FAC2_LOOP          ; loop if not all done
...
ADD_COMPARE_EXP:
    tay                         ; copy exponent
    beq WAIT_EXIT_RTS           ; exit if zero

    sec                         ; set carry for subtract
    sbc FAC1_e                  ; subtract FAC1 exponent
    beq ADD_DO_MANTISSAS        ; branch if = (go add mantissa)

    bcc ADD_FAC1_SMALLER        ; branch if <
...
; FAC2>FAC1
    ...
    bne ADD_RANGE_CHK           ; branch always
...
NORM_FAC1_ABS:
    bcs NORM_FAC1               ; branch if number is +ve

    jsr NEGATE_FAC1             ; negate FAC1
...
NORM_FAC1:
    ...
    tya                         ; clear A
    clc                         ; clear carry for add
NORM_FAC1_SHIFT_LOOP:
    ldx FAC1_1                  ; get FAC1 mantissa1
    bne NORM_CHK_MANTISSA1      ; if not zero normalise FAC1
    ...
    bne NORM_FAC1_SHIFT_LOOP    ; loop if not max
...
ADD_MANTISSAS:
    ...
    sta FAC1_1                  ; save FAC1 mantissa1
    bcs NORM_INC_EXP            ; if carry then normalise FAC1 for C=1
    rts                         ; else just exit
...
; normalise FAC1
NORM_CHK_MANTISSA1:
    bpl NORM_SHIFT_LEFT         ; loop if not normalised

    sec                         ; set carry for subtract
    sbc FAC1_e                  ; subtract FAC1 exponent
    bcs CLEAR_FAC1_EXP          ; branch if underflow (set result = $0)
...
NORM_FOR_CARRY:
    bcc NORM_DONE               ; exit if no overflow

; normalise FAC1 for C=1
NORM_INC_EXP:
    inc FAC1_e                  ; increment FAC1 exponent
    beq OVERFLOW_ERR            ; if zero do overflow error and warm start
...
TWOS_COMP_FAC1:
    ...
    bne TWOS_COMP_DONE          ; exit if no overflow

; increment FAC1 mantissa
INC_FAC1_MANTISSA:
    inc FAC1_3                  ; increment FAC1 mantissa3
    bne TWOS_COMP_DONE          ; finished if no rollover
...
SHIFT_FACX_RIGHT:
    ...
    bmi SHIFT_FACX_INIT         ; go do 8 shift if still -ve

    beq SHIFT_FACX_INIT         ; go do 8 shift if zero
...
SHIFT_FACX_BYTE:
    ...
    bcc SHIFT_FACX_ROR          ; branch if +ve
...
SHIFT_FACX_Y_TIMES:
    ...
    bne SHIFT_FACX_BYTE         ; branch if range adjust not complete
...
LAB_LOG:
    jsr GET_FAC1_SIGN           ; test sign and zero
    beq LOG_ZERO_ERR            ; if zero do function call error then warm start

    bpl LOG_POSITIVE            ; skip error if +ve
...
LOG_POSITIVE:
    ...
    sta FAC1_e                  ; save FAC1 exponent
    lda #<LOG_INVSQRT2          ; set 1/root2 pointer low byte
    ldy #>LOG_INVSQRT2          ; set 1/root2 pointer high byte
    jsr ADD_AY_TO_FAC1          ; add (AY) to FAC1 (1/root2)
    lda #<LOG_SQRT2             ; set root2 pointer low byte
    ldy #>LOG_SQRT2             ; set root2 pointer high byte
    jsr DIVIDE_INTO_AY          ; convert AY and do (AY)/FAC1 (root2/(x+(1/root2)))
    lda #<CONST_ONE             ; set 1 pointer low byte
    ldy #>CONST_ONE             ; set 1 pointer high byte
    jsr SUBTRACT_FROM_AY        ; subtract (AY) from FAC1 ((root2/(x+(1/root2)))-1)
    lda #<LOG_SERIES_COUNTER    ; set pointer low byte to counter
    ldy #>LOG_SERIES_COUNTER    ; set pointer high byte to counter
    jsr SERIES_SQR_EVAL         ; ^2 then series evaluation
    lda #<LOG_NEG_HALF          ; set -0.5 pointer low byte
    ldy #>LOG_NEG_HALF          ; set -0.5 pointer high byte
    jsr ADD_AY_TO_FAC1          ; add (AY) to FAC1
    pla                         ; restore FAC1 exponent
    jsr STR_EVAL_ASCII_DIGIT    ; evaluate new ASCII digit
    lda #<LOG_LOG2              ; set LOG(2) pointer low byte
    ldy #>LOG_LOG2              ; set LOG(2) pointer high byte
...
CONV_AY_MULTIPLY:
    jsr UNPACK_AY_TO_FAC2       ; unpack memory (AY) into FAC2
LAB_MULTIPLY:
    beq MUL_RTS                 ; exit if zero

    jsr TEST_ADJ_FAC2_EXP       ; test and adjust accumulators
    lda #$00                    ; clear A
    sta FACt_1                  ; clear temp mantissa1
    sta FACt_2                  ; clear temp mantissa2
    sta FACt_3                  ; clear temp mantissa3
    lda FAC1_r                  ; get FAC1 rounding byte
    jsr MUL_BYTE_SHIFT          ; go do shift/add FAC2
    lda FAC1_3                  ; get FAC1 mantissa3
    jsr MUL_BYTE_SHIFT          ; go do shift/add FAC2
    lda FAC1_2                  ; get FAC1 mantissa2
    jsr MUL_BYTE_SHIFT          ; go do shift/add FAC2
    lda FAC1_1                  ; get FAC1 mantissa1
    jsr MUL_SHIFT_ADD           ; go do shift/add FAC2
    jmp COPY_TEMP_NORM_FAC1     ; copy temp to FAC1, normalise and return

MUL_BYTE_SHIFT:
    bne MUL_SHIFT_ADD           ; branch if byte <> zero
; *** begin patch  2.22p5.6  floating point multiply rounding bug
; *** replace
;      JMP   SHIFT_FACX_8_LEFT          ; shift FCAtemp << A+8 times
;
...
; *** with
    sec
    jmp SHIFT_FACX_8_LEFT       ; shift FACtemp << A+8 times
...
MUL_TEST_BIT:
    tay                         ; copy result
    bcc MUL_SHIFT_TEMP          ; skip next if bit was zero
...
MUL_SHIFT_TEMP:
    ...
    tya                         ; get byte back
    lsr                         ; shift byte
    bne MUL_TEST_BIT            ; loop if all bits not done
...
TEST_ADJ_FAC2_EXP:
    lda FAC2_e                  ; get FAC2 exponent
TEST_ADJ_FAC2_NZERO:
    beq TEST_ADJ_UNDERFLOW      ; branch if FAC2 = $00 (handle underflow)

    clc                         ; clear carry for add
    adc FAC1_e                  ; add FAC1 exponent
    bcc TEST_ADJ_UNDERFLOW_CHK  ; branch if sum of exponents <$0100

    bmi OVERFLOW_ERR_JMP        ; do overflow error

    clc                         ; clear carry for the add
      .byte $2C               ; makes next line BIT $1410
TEST_ADJ_UNDERFLOW_CHK:
    bpl TEST_ADJ_UNDERFLOW      ; if +ve go handle underflow

    adc #$80                    ; adjust exponent
    sta FAC1_e                  ; save FAC1 exponent
    bne TEST_ADJ_SAVE_SIGN      ; branch if not zero

    jmp SAVE_FAC1_SIGN          ; save FAC1 sign and return
...
TEST_ADJ_OVERFLOW:
    lda FAC1_s                  ; get FAC1 sign (b7)
    bpl OVERFLOW_ERR_JMP        ; do overflow error
...
MULTIPLY_BY_10:
    jsr ROUND_COPY_FAC1_FAC2    ; round and copy FAC1 to FAC2
    tax                         ; copy exponent (set the flags)
    beq TEST_ADJ_RTS            ; exit if zero

    clc                         ; clear carry for add
    adc #$02                    ; add two to exponent (*4)
    bcs OVERFLOW_ERR_JMP        ; do overflow error if > $FF

    ldx #$00                    ; clear byte
    stx FAC_sc                  ; clear sign compare (FAC1 EOR FAC2)
    jsr ADD_COMPARE_EXP         ; add FAC2 to FAC1 (*5)
    inc FAC1_e                  ; increment FAC1 exponent (*10)
    bne TEST_ADJ_RTS            ; if non zero just do RTS

OVERFLOW_ERR_JMP:
    jmp OVERFLOW_ERR            ; do overflow error and warm start

; divide by 10

DIVIDE_BY_10:
    jsr ROUND_COPY_FAC1_FAC2    ; round and copy FAC1 to FAC2
    lda #<CONST_TEN             ; set pointer to 10d low addr
    ldy #>CONST_TEN             ; set pointer to 10d high addr
    ldx #$00                    ; clear sign
...
; Perform divide-into
LAB_DIVIDE:
    beq DIV_BY_ZERO_ERR         ; if zero go do /0 error
    ...
    jsr TEST_ADJ_FAC2_EXP       ; test and adjust accumulators
    inc FAC1_e                  ; increment FAC1 exponent
    beq OVERFLOW_ERR_JMP        ; if zero do overflow error
...
DIV_COMPARE_LOOP:
    ...
    bne DIV_COMPARE_MANTISSA    ; branch if <>
...
DIV_COMPARE_MANTISSA:
    php                         ; save FAC2-FAC1 compare status
    rol                         ; shift the result byte
    bcc DIV_RESTORE_STATUS      ; if no carry skip the byte save

    ldy #$01                    ; set bit to flag byte save
    inx                         ; else increment the index to FACt
    cpx #$02                    ; compare with the index to FACt_3
    bmi DIV_SAVE_BYTE           ; if not last byte just go save it

    bne DIV_SAVE_ROUND_NORM     ; if all done go save FAC1 rounding byte, normalise and
; return
...
DIV_RESTORE_STATUS:
    ...
    bcc DIV_FAC2_TIMES_2        ; if FAC2 < FAC1 then skip the subtract
...
DIV_FAC2_TIMES_2:
    ...
    bcs DIV_COMPARE_MANTISSA    ; loop with no compare

    bmi DIV_COMPARE_LOOP        ; loop with compare

    bpl DIV_COMPARE_MANTISSA    ; loop always with no compare
...
COPY_TEMP_NORM_FAC1:
    ...
    lda FACt_3                  ; get temp mantissa3
    sta FAC1_3                  ; save FAC1 mantissa3
    jmp NORM_FAC1               ; normalise FAC1 and return
...
PACK_FAC1_ADATAH:
    ...
    beq PACK_FAC1_XY            ; pack FAC1 into (XY) and return
...
COPY_FAC1_FAC2_LOOP:
    ...
    dex                         ; decrement count
    bne COPY_FAC1_FAC2_LOOP     ; loop if not all done
...
ROUND_FAC1:
    lda FAC1_e                  ; get FAC1 exponent
    beq COPY_FAC_RTS            ; exit if zero

    asl FAC1_r                  ; shift FAC1 rounding byte
    bcc COPY_FAC_RTS            ; exit if no overflow

; round FAC1 (no check)

ROUND_FAC1_NO_CHK:
    jsr INC_FAC1_MANTISSA       ; increment FAC1 mantissa
    bne COPY_FAC_RTS            ; branch if no overflow

    jmp NORM_INC_EXP            ; normalise FAC1 for C=1 and return
...
GET_FAC1_SIGN:
    lda FAC1_e                  ; get FAC1 exponent
    beq GET_FAC1_SIGN_RTS       ; exit if zero (already correct SGN(0)=0)

; return A=FF,C=1/-ve A=01,C=0/+ve
; no = 0 check

GET_FAC1_SIGN_NOSIGN:
...
GET_FAC1_SIGN_INREG:
    ...
    bcs GET_FAC1_SIGN_RTS       ; return if sign was set (-ve)
...
LAB_SGN:
    jsr GET_FAC1_SIGN           ; get FAC1 sign
...
LAB_STFA:
    ...
    jmp NORM_FAC1_ABS           ; do ABS and normalise FAC1
...
; A=01,C=0/+ve and return
    ...
    sbc FAC1_3                  ; subtract FAC1 mantissa3
    beq FLOAT_FIX_NORM_RTS      ; exit if mantissa3 equal

; gets here if number <> FAC1

FAC_CMP_DIFFER:
    lda FAC1_s                  ; get FAC1 sign (b7)
    bcc FAC_CMP_SIGN_RET        ; branch if FAC1 > (AY)

    eor #$FF                    ; else toggle FAC1 sign
FAC_CMP_SIGN_RET:
    jmp GET_FAC1_SIGN_INREG     ; return A=FF,C=1/-ve A=01,C=0/+ve

; convert FAC1 floating-to-fixed

FLOAT_TO_FIXED:
    lda FAC1_e                  ; get FAC1 exponent
    beq FAC1_CLEAR_RTS          ; if zero go clear FAC1 and return

    sec                         ; set carry for subtract
    sbc #$98                    ; subtract maximum integer range exponent
    bit FAC1_s                  ; test FAC1 sign (b7)
    bpl FLOAT_FIX_PLUS          ; branch if FAC1 +ve

; FAC1 was -ve
    tax                         ; copy subtracted exponent
    lda #$FF                    ; overflow for -ve number
    sta FAC1_o                  ; set FAC1 overflow byte
    jsr TWOS_COMP_FAC1          ; twos complement FAC1 mantissa
    txa                         ; restore subtracted exponent
FLOAT_FIX_PLUS:
    ldx #FAC1_e                 ; set index to FAC1
    cmp #$F9                    ; compare exponent result
    bpl SHIFT_FAC1_A_RIGHT      ; if < 8 shifts shift FAC1 A times right and return

    jsr SHIFT_FACX_RIGHT        ; shift FAC1 A times right (> 8 shifts)
...
SHIFT_FAC1_A_RIGHT:
    ...
    sta FAC1_1                  ; save FAC1 mantissa1
    jsr SHIFT_FACX_Y_TIMES      ; shift FAC1 Y times right
    sty FAC1_o                  ; clear FAC1 overflow byte
    rts
...
LAB_INT:
    ...
    bcs STR_TO_FAC1_RTS         ; exit if >= (already int, too big for fractional part!)
...
STR_TO_FAC1_LOOP:
    ...
    dex                         ; decrement index
    bpl STR_TO_FAC1_LOOP        ; loop until numexp to negnum (and FAC1) = $00

    bcc STR_FAC1_TIMES10_ADD    ; branch if 1st character numeric

; get FAC1 from string .. first character wasn't numeric

    cmp #'-'                    ; else compare with "-"
    bne STR_TO_FAC1_NOT_MINUS   ; branch if not "-"

    stx negnum                  ; set flag for -ve number (X = $FF)
    beq STR_TO_FAC1_NEXT        ; branch always (go scan and check for hex/bin)

; get FAC1 from string .. first character wasn't numeric or -

STR_TO_FAC1_NOT_MINUS:
    cmp #'+'                    ; else compare with "+"
    bne STR_TO_FAC1_HEX_BIN     ; branch if not "+" (go check for hex/bin)

; was "+" or "-" to start, so get next character

STR_TO_FAC1_NEXT:
    jsr LAB_IGBY                ; increment and scan memory
    bcc STR_FAC1_TIMES10_ADD    ; branch if numeric character
...
LAB_NHEX:
    cmp #'%'                    ; else compare with "%"
    bne STR_TO_FAC1_CHK_DOT     ; branch if not "%" (continue original code)

    jmp LAB_CBIN                ; branch if "%"

STR_TO_FAC1_NEXT_CHR:
...
STR_TO_FAC1_CHK_DOT:
    cmp #'.'                    ; else compare with "."
    beq STR_SET_DECIMAL_FLAG    ; branch if "."

; get FAC1 from string .. character wasn't numeric, -, + or .

    cmp #'E'                    ; else compare with "E"
    bne STR_EVAL_EXPONENT       ; branch if not "E"

; was "E" so evaluate exponential part
    jsr LAB_IGBY                ; increment and scan memory
    bcc STR_EXP_CHK_NUM         ; branch if numeric character

    cmp #token_MINUS            ; else compare with token for -
    beq STR_EXP_SET_NEG         ; branch if token for -

    cmp #'-'                    ; else compare with "-"
    beq STR_EXP_SET_NEG         ; branch if "-"

    cmp #token_PLUS             ; else compare with token for +
    beq STR_EXP_NEXT_CHR        ; branch if token for +

    cmp #'+'                    ; else compare with "+"
    beq STR_EXP_NEXT_CHR        ; branch if "+"

    bne STR_EXP_CHK_SIGN        ; branch always
...
STR_EXP_NEXT_CHR:
    jsr LAB_IGBY                ; increment and scan memory
STR_EXP_CHK_NUM:
    bcc STR_EVAL_EXP_DIGIT      ; branch if numeric character

STR_EXP_CHK_SIGN:
    bit expneg                  ; test exponent -ve flag
    bpl STR_EVAL_EXPONENT       ; if +ve go evaluate exponent

; else do exponent = -exponent
    lda #$00                    ; clear result
    sec                         ; set carry for subtract
    sbc expcnt                  ; subtract exponent byte
    jmp STR_ADJ_EXPONENT        ; go evaluate exponent

STR_SET_DECIMAL_FLAG:
    ror numdpf                  ; set decimal point flag
    bit numdpf                  ; test decimal point flag
    bvc STR_TO_FAC1_NEXT_CHR    ; branch if only one decimal point so far
...
STR_ADJ_EXPONENT:
    ...
    beq STR_CHK_NEG             ; branch if no adjustment

    bpl STR_MUL_EXPCNT_LOOP     ; else if +ve go do FAC1*10^expcnt

; else go do FAC1/10^(0-expcnt)
STR_DIV_EXPCNT_LOOP:
    jsr DIVIDE_BY_10            ; divide by 10
    inc expcnt                  ; increment exponent count byte
    bne STR_DIV_EXPCNT_LOOP     ; loop until all done

    beq STR_CHK_NEG             ; branch always

STR_MUL_EXPCNT_LOOP:
    jsr MULTIPLY_BY_10          ; multiply by 10
    dec expcnt                  ; decrement exponent count byte
    bne STR_MUL_EXPCNT_LOOP     ; loop until all done

STR_CHK_NEG:
    lda negnum                  ; get -ve flag
    bmi STR_DO_NEGATE           ; if -ve do - FAC1 and return
...
STR_FAC1_TIMES10_ADD:
    pha                         ; save character
    bit numdpf                  ; test decimal point flag
    bpl STR_MUL10_DIGIT         ; skip exponent increment if not set

    inc numexp                  ; else increment number exponent
STR_MUL10_DIGIT:
    jsr MULTIPLY_BY_10          ; multiply FAC1 by 10
    pla                         ; restore character
    and #$0F                    ; convert to binary
    jsr STR_EVAL_ASCII_DIGIT    ; evaluate new ASCII digit
    jmp STR_TO_FAC1_NEXT_CHR    ; go do next character

; evaluate new ASCII digit

STR_EVAL_ASCII_DIGIT:
    pha                         ; save digit
    jsr ROUND_COPY_FAC1_FAC2    ; round and copy FAC1 to FAC2
    pla                         ; restore digit
    jsr SAVE_A_AS_INT           ; save A as integer byte
    lda FAC2_s                  ; get FAC2 sign (b7)
...
STR_EVAL_EXP_DIGIT:
    ...
    bcc STR_CALC_EXP            ; branch if less

    lda #$64                    ; make all -ve exponents = -100 decimal (causes underflow)
    bit expneg                  ; test exponent -ve flag
    bmi STR_SAVE_EXPCNT         ; branch if -ve

    jmp OVERFLOW_ERR            ; else do overflow error
...
STR_SAVE_EXPCNT:
    sta expcnt                  ; save exponent count byte
    jmp STR_EXP_NEXT_CHR        ; go get next character
...
PRINT_XA_UNSIGNED:
    ...
    ldy #$00                    ; clear index
    tya                         ; clear A
    jsr FAC1_TO_STR_BODY        ; convert FAC1 to string, skip sign character save
    jmp PRINT_STR_FROM_AY       ; print null terminated string from memory and return
...
FAC1_TO_STR_CSTR:
    ...
    bit FAC1_s                  ; test FAC1 sign (b7)
    bmi FAC1_TO_STR_MINUS       ; branch if -ve
    ldy #$00
    lda #$00
    bra FAC1_TO_STR_BODY
FAC1_TO_STR_STR:
    ldy #$01                    ; set index = 1
    lda #$20                    ; character = " " (assume +ve)
    bit FAC1_s                  ; test FAC1 sign (b7)
    bpl FAC1_TO_STR_SIGN        ; branch if +ve
...
FAC1_TO_STR_BODY:
    ...
    iny                         ; increment index
    ldx FAC1_e                  ; get FAC1 exponent
    bne FAC1_TO_STR_NONZERO     ; branch if FAC1<>0

; exponent was $00 so FAC1 is 0
    lda #'0'                    ; set character = "0"
    jmp FAC1_TO_STR_LAST_CHR    ; save last character, [EOT] and exit
...
FAC1_TO_STR_NONZERO:
    ...
    bcs FAC1_TO_STR_SAVE_EXP    ; branch if FAC1=>1

; FAC1<1
    lda #<PRINT_ONE_MILLION     ; set pointer low byte to 1,000,000
    ldy #>PRINT_ONE_MILLION     ; set pointer high byte to 1,000,000
    jsr CONV_AY_MULTIPLY        ; do convert AY, FCA1*(AY)
    lda #$FA                    ; set number exponent count (-6)
FAC1_TO_STR_SAVE_EXP:
    sta numexp                  ; save number exponent count
FAC1_TO_STR_CHK_MAX:
    lda #<PRINT_MAX_BEFORE_SCI  ; set pointer low byte to 999999.4375 (max before sci note)
    ldy #>PRINT_MAX_BEFORE_SCI  ; set pointer high byte to 999999.4375
    jsr CMP_FAC1_AY             ; compare FAC1 with (AY)
    beq FAC1_TO_STR_FIX         ; exit if FAC1 = (AY)

    bpl FAC1_TO_STR_DIV10       ; go do /10 if FAC1 > (AY)

; FAC1 < (AY)
FAC1_TO_STR_CHK_MIN:
    lda #<PRINT_MAX_1DEC        ; set pointer low byte to 99999.9375
    ldy #>PRINT_MAX_1DEC        ; set pointer high byte to 99999.9375
    jsr CMP_FAC1_AY             ; compare FAC1 with (AY)
    beq FAC1_TO_STR_MUL10       ; branch if FAC1 = (AY) (allow decimal places)

    bpl FAC1_TO_STR_ROUND       ; branch if FAC1 > (AY) (no decimal places)

; FAC1 <= (AY)
FAC1_TO_STR_MUL10:
    jsr MULTIPLY_BY_10          ; multiply by 10
    dec numexp                  ; decrement number exponent count
    bne FAC1_TO_STR_CHK_MIN     ; go test again (branch always)

FAC1_TO_STR_DIV10:
    jsr DIVIDE_BY_10            ; divide by 10
    inc numexp                  ; increment number exponent count
    bne FAC1_TO_STR_CHK_MAX     ; go test again (branch always)

; now we have just the digits to do

FAC1_TO_STR_ROUND:
    jsr ADD_HALF_TO_FAC1        ; add 0.5 to FAC1 (round FAC1)
FAC1_TO_STR_FIX:
    jsr FLOAT_TO_FIXED          ; convert FAC1 floating-to-fixed
    ...
    bmi FAC1_TO_STR_SET_CARRY   ; if -ve then 1 digit before dp

    cmp #$08                    ; A>=8 if n>=1E6
    bcs FAC1_TO_STR_ADJ_EXP     ; branch if >= $08

; carry is clear
    adc #$FF                    ; take 1 from digit count
    tax                         ; copy to A
    lda #$02                    ;.set exponent adjust
FAC1_TO_STR_SET_CARRY:
    sec                         ; set carry for subtract
FAC1_TO_STR_ADJ_EXP:
    sbc #$02                    ; -2
    ...
    txa                         ; copy to A
    beq FAC1_TO_STR_NO_INT_DIG  ; branch if no digits before dp

    bpl FAC1_TO_STR_DIGIT_LOOP  ; branch if digits before dp

FAC1_TO_STR_NO_INT_DIG:
    ldy Sendl                   ; get output string index
    lda #$2E                    ; character "."
    iny                         ; increment index
    sta Decss,Y                 ; save to output string
    txa                         ;.
    beq FAC1_TO_STR_SAVE_IDX    ;.

    lda #'0'                    ; character "0"
    iny                         ; increment index
    sta Decss,Y                 ; save to output string
FAC1_TO_STR_SAVE_IDX:
    sty Sendl                   ; save output string index
FAC1_TO_STR_DIGIT_LOOP:
    ldy #$00                    ; clear index (point to 100,000)
    ldx #$80                    ;
FAC1_TO_STR_SUBLOOP:
    lda FAC1_3                  ; get FAC1 mantissa3
    clc                         ; clear carry for add
    adc PRINT_DEC_LSB,Y         ; add -ve LSB
    sta FAC1_3                  ; save FAC1 mantissa3
    lda FAC1_2                  ; get FAC1 mantissa2
    adc PRINT_DEC_NMSB,Y        ; add -ve NMSB
    sta FAC1_2                  ; save FAC1 mantissa2
    lda FAC1_1                  ; get FAC1 mantissa1
    adc PRINT_DEC_TABLE,Y       ; add -ve MSB
    sta FAC1_1                  ; save FAC1 mantissa1
    inx                         ;
    bcs FAC1_TO_STR_CARRY_POS   ;

    bpl FAC1_TO_STR_SUBLOOP     ; not -ve so try again

    bmi FAC1_TO_STR_GET_DIGIT   ;

FAC1_TO_STR_CARRY_POS:
    bmi FAC1_TO_STR_SUBLOOP     ;

FAC1_TO_STR_GET_DIGIT:
    txa                         ;
    bcc FAC1_TO_STR_SAVE_DIGIT  ;

    eor #$FF                    ;
    adc #$0A                    ;
FAC1_TO_STR_SAVE_DIGIT:
    ...
    bne FAC1_TO_STR_NEXT_DIGIT  ; branch if still characters to do

; else output the point
    lda #$2E                    ; character "."
    iny                         ; increment output string index
    sta Decss,Y                 ; save to output string
FAC1_TO_STR_NEXT_DIGIT:
    sty Sendl                   ; save output string index
    ...
    bne FAC1_TO_STR_SUBLOOP     ; loop if not max

; now remove trailing zeroes
    ldy Sendl                   ; get output string index
FAC1_TO_STR_STRIP_ZERO:
    lda Decss,Y                 ; get character from output string
    dey                         ; decrement output string index
    cmp #'0'                    ; compare with "0"
    beq FAC1_TO_STR_STRIP_ZERO  ; loop until non "0" character found

    cmp #'.'                    ; compare with "."
    beq FAC1_TO_STR_EXP_SIGN    ; branch if was dp

; restore last character
    iny                         ; increment output string index
FAC1_TO_STR_EXP_SIGN:
    lda #$2B                    ; character "+"
    ldx expcnt                  ; get exponent count
    beq FAC1_TO_STR_NULL_TERM   ; if zero go set null terminator and exit

; exponent isn't zero so write exponent
    bpl FAC1_TO_STR_EXP_PLUS    ; branch if exponent count +ve
...
FAC1_TO_STR_EXP_TENS:
    ...
    beq FAC1_TO_STR_SET_PTR     ; go set string pointer (AY) and exit (branch always)

; save last character, [EOT] and exit
FAC1_TO_STR_LAST_CHR:
    sta Decss,Y                 ; save last character to output string

; set null terminator and exit
FAC1_TO_STR_NULL_TERM:
    lda #$00                    ; set null terminator
...
LAB_POWER:
    ...
    lda FAC2_e                  ; get FAC2 exponent
    bne POWER_FAC2_NONZERO      ; branch if FAC2<>0

    jmp CLEAR_FAC1_EXP2         ; clear FAC1 exponent and sign and return

POWER_FAC2_NONZERO:
    ldx #<func_l                ; set destination pointer low byte
    ldy #>func_l                ; set destination pointer high byte
    jsr PACK_FAC1_XY            ; pack FAC1 into (XY)
    lda FAC2_s                  ; get FAC2 sign (b7)
    bpl POWER_DO_LOG            ; branch if FAC2>0
...
; integer power which gives an x +j0 result
    ...
    jsr CMP_FAC1_AY             ; compare FAC1 with (AY)
    bne POWER_DO_LOG            ; branch if FAC1 <> (AY) to allow Function Call error
...
LAB_GTHAN:
    lda FAC1_e                  ; get FAC1 exponent
    beq POWER_NEGATE_RTS        ; exit if FAC1_e = $00
...
LAB_EXP:
    lda #<EXP_1_OVER_LOG2E      ; set 1.443 pointer low byte
    ldy #>EXP_1_OVER_LOG2E      ; set 1.443 pointer high byte
    jsr CONV_AY_MULTIPLY        ; do convert AY, FCA1*(AY)
    lda FAC1_r                  ; get FAC1 rounding byte
    adc #$50                    ; +$50/$100
    bcc EXP_SAVE_FAC2R          ; skip rounding if no carry

    jsr ROUND_FAC1_NO_CHK       ; round FAC1 (no check)
EXP_SAVE_FAC2R:
    sta FAC2_r                  ; save FAC2 rounding byte
    jsr COPY_FAC1_TO_FAC2       ; copy FAC1 to FAC2
    lda FAC1_e                  ; get FAC1 exponent
    cmp #$88                    ; compare with EXP limit (256d)
    bcc EXP_DO_INT              ; branch if less

EXP_OVERFLOW_CHK:
    jsr TEST_ADJ_OVERFLOW       ; handle overflow and underflow
EXP_DO_INT:
    jsr LAB_INT                 ; perform INT
    lda Temp3                   ; get mantissa 3 from INT() function
    clc                         ; clear carry for add
    adc #$81                    ; normalise +1
    beq EXP_OVERFLOW_CHK        ; if $00 go handle overflow
...
EXP_SWAP_FACS_LOOP:
    ...
    bpl EXP_SWAP_FACS_LOOP      ; loop if not all done
    ...
    lda #<EXP_SERIES_COUNTER    ; set counter pointer low byte
    ldy #>EXP_SERIES_COUNTER    ; set counter pointer high byte
    jsr SERIES_EVAL             ; go do series evaluation
    lda #$00                    ; clear A
    sta FAC_sc                  ; clear sign compare (FAC1 EOR FAC2)
    pla                         ;.get saved FAC2 exponent
    jmp TEST_ADJ_FAC2_NZERO     ; test and adjust accumulators and return
...
SERIES_SQR_EVAL:
    ...
    jsr PACK_FAC1_ADATAL        ; pack FAC1 into Adatal
    lda #<Adatal                ; set pointer low byte (Y already $00)
    jsr CONV_AY_MULTIPLY        ; do convert AY, FCA1*(AY)
    jsr SERIES_EVAL_BODY        ; go do series evaluation
    lda #<Adatal                ; pointer to original # low byte
    ldy #>Adatal                ; pointer to original # high byte
    jmp CONV_AY_MULTIPLY        ; do convert AY, FCA1*(AY) and return
...
SERIES_EVAL_BODY:
    ...
    jsr PACK_FAC1_ADATAH        ; set pointer high byte and pack FAC1 into numexp
    ...
    tya                         ; copy it
    bne SERIES_SKIP_INC         ; skip next if no overflow
...
SERIES_EVAL_LOOP:
    ...
    bcc SERIES_ADV_PTR          ; skip next if no overflow
...
SERIES_ADV_PTR:
    ...
    bne SERIES_EVAL_LOOP        ; loop until all done
...
; perform COS()
LAB_COS:
    lda #<CONST_PI_OVER_2       ; set (pi/2) pointer low byte
    ldy #>CONST_PI_OVER_2       ; set (pi/2) pointer high byte
    jsr ADD_AY_TO_FAC1          ; add (AY) to FAC1

; perform SIN()
LAB_SIN:
    jsr ROUND_COPY_FAC1_FAC2    ; round and copy FAC1 to FAC2
    lda #<CONST_TWO_PI          ; set (2*pi) pointer low byte
    ldy #>CONST_TWO_PI          ; set (2*pi) pointer high byte
    ldx FAC2_s                  ; get FAC2 sign (b7)
    jsr DIVIDE_BY_AY            ; divide by (AY) (X=sign)
    jsr ROUND_COPY_FAC1_FAC2    ; round and copy FAC1 to FAC2
    jsr LAB_INT                 ; perform INT
    lda #$00                    ; clear byte
    sta FAC_sc                  ; clear sign compare (FAC1 EOR FAC2)
    jsr LAB_SUBTRACT            ; perform subtraction, FAC2 from FAC1
    lda #<CONST_QUARTER         ; set 0.25 pointer low byte
    ldy #>CONST_QUARTER         ; set 0.25 pointer high byte
    jsr SUBTRACT_FROM_AY        ; perform subtraction, (AY) from FAC1
    lda FAC1_s                  ; get FAC1 sign (b7)
    pha                         ; save FAC1 sign
    bpl TRIG_DO_NEG_FAC1        ; branch if +ve

; FAC1 sign was -ve
    jsr ADD_HALF_TO_FAC1        ; add 0.5 to FAC1
    lda FAC1_s                  ; get FAC1 sign (b7)
    bmi TRIG_ADD_QUARTER        ; branch if -ve
...
TRIG_ADD_QUARTER:
    ...
    bpl TRIG_SIN_SERIES         ; branch if was +ve

; else correct FAC1
    jsr LAB_GTHAN               ; do - FAC1
TRIG_SIN_SERIES:
    lda #<CONST_SIN_COUNTER     ; set pointer low byte to counter
    ldy #>CONST_SIN_COUNTER     ; set pointer high byte to counter
    jmp SERIES_SQR_EVAL         ; ^2 then series evaluation and return
...
LAB_TAN:
    ...
    sta FAC1_s                  ; clear FAC1 sign (b7)
    lda Cflag                   ; get comparison evaluation flag
    jsr TAN_SAVE_FLAG_SERIES    ; save flag and go do series evaluation

    lda #<func_l                ; set sin(n) pointer low byte
    ldy #>func_l                ; set sin(n) pointer high byte
    jmp DIVIDE_INTO_AY          ; convert AY and do (AY)/FAC1

TAN_SAVE_FLAG_SERIES:
    pha                         ; save comparison evaluation flag
    jmp TRIG_DO_NEG_FAC1        ; go do series evaluation

; perform USR()
LAB_USR:
    jsr Usrjmp                  ; call user code
    jmp SCAN_CLOSE_PAREN        ; scan for ")", else do syntax error then warm start

; perform ATN()
LAB_ATN:
    lda FAC1_s                  ; get FAC1 sign (b7)
    pha                         ; save sign
    bpl ATN_POSITIVE            ; branch if +ve
...
ATN_POSITIVE:
    ...
    bcc ATN_SERIES              ; branch if FAC1<1

    lda #<CONST_ONE             ; set 1 pointer low byte
    ldy #>CONST_ONE             ; set 1 pointer high byte
    jsr DIVIDE_INTO_AY          ; convert AY and do (AY)/FAC1
ATN_SERIES:
    lda #<CONST_ATN_COUNTER     ; set pointer low byte to counter
    ldy #>CONST_ATN_COUNTER     ; set pointer high byte to counter
    jsr SERIES_SQR_EVAL         ; ^2 then series evaluation
    pla                         ; restore old FAC1 exponent
    cmp #$81                    ; compare with 1
    bcc ATN_CHK_RECIP           ; branch if FAC1<1
...
LAB_NOTT:
    jmp SAVE_A_AS_INT           ; go do SGN tail
...
LAB_EXCH:
    jmp STR_CHK_NEG             ; evaluate -ve flag and return
...
CTRLC:
    ...
    bne CTRLC_CHK_INTERRUPT     ; exit if inhibited

    jsr V_INPT                  ; scan input device
    bcc CTRLC_NO_BYTE           ; exit if buffer empty

    sta ccbyte                  ; save received byte
    ldx #$20                    ; "life" timer for bytes
    stx ccnull                  ; set countdown
    jmp CTRLC_KEY_DETECTED      ; return to BASIC

CTRLC_NO_BYTE:
    ldx ccnull                  ; get countdown byte
    beq CTRLC_CHK_INTERRUPT     ; exit if finished
...
INGET:
    jsr V_INPT                  ; call scan input device
    bcs INGET_CLEAR_TIMER       ; if byte go reset timer

    lda ccnull                  ; get countdown
    beq INGET_RTS               ; exit if empty
...
LAB_SNMI:
    ...
    jmp UNDEF_STMT_ERROR        ; else go do "Undefined statement" error and warm start
...
LAB_RETIRQ:
    ...
    jmp RETURN_POP_CTX          ; go do rest of RETURN
...
; pull FAC2 and compare with FAC1
    ...
    sta FAC2_1                  ; save FAC2 mantissa1
    jsr COPY_FAC2_TO_FAC1       ; copy FAC2 to FAC1
...
; compare FAC1 with (packed) FAC2
    lda #<FAC2_e                ; set pointer low byte to FAC2
    ldy #>FAC2_e                ; set pointer high byte to FAC2
    jmp CMP_FAC1_AY             ; compare FAC1 with FAC2 (AY) and return
; returns A=$00 if FAC1 = (AY)
; returns A=$01 if FAC1 > (AY)
; returns A=$FF if FAC1 < (AY)
...
; the target address for the LDA at ZP_SCAN_MEMORY becomes the BASIC execute pointer once the
...
ZP_INC_BPNTRL:
    ...
    bne ZP_SCAN_MEMORY          ; branch if no carry
; else
...
ZP_SCAN_MEMORY:
    ...
    beq ZP_SCAN_RTS             ; exit if ELSE, not numeric, carry set

    cmp   #':'              ; compare with ":"
    bcs   ZP_SCAN_RTS           ; exit if >= ":", not numeric, carry set

    cmp #' '                    ; compare with " "
    beq ZP_INC_BPNTRL           ; if " " go do next
...
CONST_NULL_WORD    = *+1             ; $00,$00 used for undefined variables
CONST_ONE:
...
PRINT_DEC_TABLE:
PRINT_DEC_NMSB = PRINT_DEC_TABLE+1
PRINT_DEC_LSB = PRINT_DEC_NMSB+1
...
LAB_FTPM = LAB_FTPL+$01
    ...
    .word GVAL_EVAL_IN_PARENS-1 ; FRE(x)          "
    .word GVAL_EVAL_IN_PARENS-1 ; POS(x)          "
...
; the above error has been tested and works (see code and comments below VAR_NOT_FOUND_CHK)
...
; This overwrites any existing program
    ...
    lda XMODEM_pstart+1
    sta Svarh
    jsr RESET_EXEC_CLEAR_VARS   ; Need to call this EhBasic routine to clear variables and reset the execution pointer
    jmp REBUILD_LINE_CHAIN      ; Jump to appropriate location in EhBasic to finish
